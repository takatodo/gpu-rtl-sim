...
`config/scaling_gates/define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` now records that bool/int and `efficiency_estimate` hardening definition and advances to review. The suffix cases still prove only parser-token hardening; this later fixture hardening remains non-executing metadata-boundary work and does not add broad parser support, sidecar execution, timing evidence, arbitrary RTL/filelist support, automatic GPU allocation, upstream regression success, or upstream landing.
