...
`config/scaling_gates/define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` records that bool/int and `efficiency_estimate` hardening definition, and `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` accepts it for implementation. The suffix cases still prove only parser-token hardening; this later fixture hardening remains non-executing metadata-boundary work and does not add broad parser support, sidecar execution, timing evidence, arbitrary RTL/filelist support, automatic GPU allocation, upstream regression success, or upstream landing.
