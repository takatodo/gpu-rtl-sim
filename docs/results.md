...
`run_hybrid_benchmark.py --operator-plan-json` provides the same target-first operator plan as JSON. It is still non-executing planning output and keeps `correctness_policy: coverage_output_equivalence` separate from the efficiency estimate.
