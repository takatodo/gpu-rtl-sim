...
The parser path has since moved from fixture and overlay-patch definition into build-only evidence. `config/scaling_gates/run_verilator_native_option_parser_overlay_patch_compile_fix_build_only_validation_gate.json` records descriptor validation, clean-checkout `git apply --check`, actual patch apply, `autoconf`, `configure`, and `make -j 28 verilator_bin` exiting `0` for the repaired repo-owned Verilator overlay patch. `config/scaling_gates/review_verilator_native_option_parser_overlay_patch_compile_fix_build_only_validation_gate.json` accepts only compile/link compatibility for that repaired patch and advances to parser-behavior boundary definition. `config/scaling_gates/define_verilator_native_option_parser_overlay_patch_parser_behavior_gate.json` defines the lint-only built-binary accept/reject smoke boundary, and `config/scaling_gates/review_verilator_native_option_parser_overlay_patch_parser_behavior_gate.json` accepts that boundary as narrow enough to execute next. This is still not native parser behavior validation, sidecar execution, timing evidence, arbitrary RTL/filelist support, automatic GPU allocation, upstream regression success, or upstream landing.
