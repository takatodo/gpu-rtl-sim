...
1. **Native parser direct command path**: Define payload-validation hardening for the accepted non-executing fixture helper before sidecar-plan materialization, execution, timing, arbitrary filelists, allocation, and broad native claims.
...
- selected next gate: `define_verilator_native_option_parser_direct_command_path_fixture_payload_validation_hardening_gate`
...
- current next task: `define_verilator_native_option_parser_direct_command_path_fixture_payload_validation_hardening_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `870`
...
- use `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_fixture_implementation_gate.json` as the source artifact
- define strict required parser-payload fields before `direct_command_parser_payload_to_sidecar_plan_fixture` can return boundary metadata
