...
1. **Native parser direct command path**: Define the execution boundary from reviewed direct command-path stage-plan metadata before any command run, compare, timing, arbitrary filelists, allocation, and broad native claims.
...
Define the direct command-path sidecar stage-plan execution boundary. The accepted implementation review allows metadata-only `sidecar_stage_plan` materialization, but the next gate must define which ready materialization fields and stage order are required before any command, compare, timing, arbitrary filelist inference, automatic allocation, or runtime/ABI change.
...
- selected next gate: `define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_boundary_gate`
...
- current next task: `define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_boundary_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `878`
...
- use `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_fixture_implementation_gate.json` as the source artifact
- define which ready materialization fixture fields may become execution-boundary inputs
- define the reviewed sidecar stage order required before any command can run
- keep generated execution evidence under `reports/` and `artifacts/`
