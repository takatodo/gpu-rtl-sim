...
Hybrid execution is close to a normal Verilator-style flow for generated templates, but "native" is still only a prototype boundary. The commit split is complete and the parser-stub fixture contract review is accepted; the weak point is now implementing that fixture without confusing it with the existing sidecar shim, tracked-template registry, arbitrary filelist planning, dependency inference, automatic allocation, or a real Verilator parser patch.

## Current Frontier

`modern_llm_serving_rtl_hybrid_conditions` is complete for the scoped RTL-harness condition-finding objective. The current work is implementing the smallest non-executing importable parser-stub fixture before any native Verilator parser implementation claim.
...
`config/scaling_gates/review_verilator_native_option_parser_stub_fixture_gate.json`
...
1. **Native option parser stub fixture implementation**: Implement the non-executing parser-stub helper, especially unknown accelerator rejection outside wrapper argparse choices, and keep public CLI, parser implementation, execution, and measurement out of scope.
...
Implement the native Verilator option parser stub fixture after accepting the fixture contract.

Parser boundary definition:

- source gate: `config/scaling_gates/review_verilator_native_option_parser_stub_fixture_gate.json`
- selected next gate: `implement_verilator_native_option_parser_stub_fixture_gate`
- scope: importable helper implementation only, not public CLI, native parser implementation, or execution
...
- review accepts the fixture contract and requires the implementation to bypass wrapper argparse choices rather than counting current CLI rejection behavior as success
...
- current next task: `implement_verilator_native_option_parser_stub_fixture_gate`
...
- use `config/scaling_gates/review_verilator_native_option_parser_stub_fixture_gate.json` as the source artifact
- add an importable non-executing helper, not a public CLI
- test accepted `sidecar-gpu` `64x1`, unknown accelerator rejection, missing/non-positive shape input rejection, compact `--sim-accel-shape` rejection, and ordinary Verilator arg preservation
- emit or return the accepted parser-only handoff fields with `coverage_output_equivalence`
