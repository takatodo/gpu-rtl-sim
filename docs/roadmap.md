...
1. **Native parser-to-sidecar execution boundary**: Run the accepted scoped execution boundary after accepted operator-plan fixture hardening, with timing and broad native claims still deferred.
...
- selected next gate: `run_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_sidecar_execution_gate`
...
- current next task: `run_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_sidecar_execution_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `862`
...
- use `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_execution_boundary_gate.json` as the source artifact
- validate ready operator-plan metadata and reviewed sidecar stage order before executing commands
- record only scoped sidecar build/run/compare evidence; do not claim native Verilator support, arbitrary filelists, automatic allocation, timing, speedup, or raw full-state equality
