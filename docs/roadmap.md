...
1. **Native parser-to-sidecar handoff-contract fixture implementation**: Add the importable non-executing helper that lets only ready plan-resolution outputs build `sidecar_handoff_contract` metadata, while fail-closing not-ready/unsupported results before any broader native parser support, upstream landing, sidecar execution, timing, or automatic allocation claim.
...
- selected next gate: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_fixture_gate`
...
- current next task: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_fixture_gate`
...
- use `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate.json` as the source artifact
- add an importable helper in `src/tools/verilator_native_option_parser_sidecar_plan_resolution.py`
- accept only `surface: native_verilator_parser_sidecar_plan_resolution_fixture`
- require `status: ready_for_verilator_option_shim`, `plan_resolution_readiness.ready_for_direct_verilator_option: true`, planned stage status, and matching parser/stage shape before calling `sidecar_handoff_contract(stage_plan)`
- fail closed for `not_ready_for_verilator_option_shim` and `unsupported_for_stage_plan` before metadata construction while preserving readiness reasons
- keep command synthesis, operator plan, execution, compare execution, timing, runtime/ABI, arbitrary filelist expansion, and automatic allocation out of scope
