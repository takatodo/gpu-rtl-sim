...
1. **Native parser-to-sidecar operator-plan fixture hardening**: Implement the reviewed bool/int strictness and `efficiency_estimate` non-timing boundary before any execution-boundary gate.
...
- selected next gate: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate`
...
- current next task: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `858`
...
- use `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` as the source artifact
- reject bool values in handoff-contract, stage-plan, hybrid-sidecar-run details, and parser-schedule integer fields before any command, estimate, efficiency, or operator-plan authority
- keep malformed Mapping handling scoped to ready handoff-contract metadata without claiming complete adversarial schema validation
- keep the `efficiency_estimate` claim boundary as planning metadata only; existing-report-derived values are not new timing or speedup evidence from this gate
