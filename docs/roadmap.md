...
1. **Native parser direct command path**: Implement the scoped launcher-invocation fixture from reviewed native-path bridge metadata to the existing sidecar launcher.
...
- selected next gate: `implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_gate`
...
- current next task: `implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_gate`
...
- use `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_boundary_gate.json` as the source artifact
- implement a scoped fixture that validates parser payload, bridge metadata, and explicit sidecar context before materializing launcher invocation
- return structured status that distinguishes bridge failure, launcher invocation construction failure, sidecar stage failure, compare failure, and timing-without-measurement
