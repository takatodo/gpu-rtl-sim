...
1. **Native parser-to-sidecar operator-plan fixture hardening**: Review the implemented bool/int strictness and `efficiency_estimate` non-timing boundary before any execution-boundary gate.
...
- selected next gate: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_implementation_gate`
...
- current next task: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_implementation_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `859`
...
- use `config/scaling_gates/implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` as the source artifact
- verify bool values in handoff-contract, stage-plan, hybrid-sidecar-run details, and parser-schedule integer fields reject before any command, estimate, efficiency, or operator-plan authority
- verify malformed Mapping handling remains scoped to ready handoff-contract metadata without claiming complete adversarial schema validation
