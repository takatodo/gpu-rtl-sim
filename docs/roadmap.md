...
- selected next gate: `review_verilator_native_option_parser_sidecar_plan_resolution_fixture_implementation_gate`
...
- current next task: `review_verilator_native_option_parser_sidecar_plan_resolution_fixture_implementation_gate`
...
- use `config/scaling_gates/implement_verilator_native_option_parser_sidecar_plan_resolution_fixture_gate.json` as the source artifact
- confirm `src/tools/verilator_native_option_parser_sidecar_plan_resolution.py` remains importable and non-executing
- confirm missing target, mode, template or registry context, and source gate/manifest ref are rejected
- confirm `sidecar_stage_plan` is reached only after explicit context exists
- confirm parser-owned values stay limited to parsed Verilator options and preserved build inputs
- confirm sidecar-owned source closure, coverage target, host-probe metadata, state/report paths, compare labels, execution, timing, and automatic allocation stay out of the parser layer
