...
1. **Native parser direct command path**: Review the scoped sidecar build/run/compare gate from reviewed direct command-path stage-plan metadata without broad native, direct command execution, timing, arbitrary filelist, allocation, or raw-state claims.
...
- selected next gate: `review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_run_gate`
...
- current next task: `review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_run_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `881`
...
- use `config/scaling_gates/run_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_gate.json` as the source artifact
- confirm the run validated ready materialization fixture metadata before execution, including `state_count` and `step_count` through `parser_schedule_constraints`
- confirm the run executed only the reviewed seven-stage sidecar build/run/compare order for the scoped target and shape
- confirm generated execution evidence stayed only under `reports/` and `artifacts/`
- accept `coverage_output_equivalence` only as scoped compare evidence for `pulp_ita_mha 64x1`
- accept no direct Verilator command execution, new measurement by the review gate, timing, speedup, runtime/ABI change, arbitrary filelist support, automatic allocation, GEM, production-serving, or raw full-state equality claim
