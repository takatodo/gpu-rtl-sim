...
1. **Native parser-to-sidecar operator-plan fixture hardening**: Review the bool/int strictness and `efficiency_estimate` non-timing boundary before any implementation or execution-boundary gate.
...
- selected next gate: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate`
...
- current next task: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `857`
...
- use `config/scaling_gates/define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_hardening_gate.json` as the source artifact
- decide whether bool rejection for handoff-contract, stage-plan, and parser-schedule integer fields is the correct minimum hardening boundary
- decide whether malformed Mapping handling is scoped narrowly enough without claiming complete adversarial schema validation
- decide whether the `efficiency_estimate` claim boundary can stay as existing planning metadata only, not new timing or speedup evidence from this gate
