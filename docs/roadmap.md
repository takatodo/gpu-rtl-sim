...
- selected next gate: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_gate`
...
- current next task: `implement_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_fixture_gate`
...
- use `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_operator_plan_boundary_gate.json` as the source artifact
- add an importable helper for accepted handoff-contract metadata to produce non-executing command/operator-plan metadata
- call existing command argv, estimate-command, efficiency-estimate, and operator-plan authorities only after ready-only checks pass
- reject closed, unsupported, malformed, or prepopulated handoff-contract metadata before command/operator-plan metadata construction
