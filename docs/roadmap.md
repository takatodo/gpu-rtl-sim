...
1. **Native parser direct command path**: Review how the validated direct command-path fixture can materialize or reference the reviewed `sidecar_stage_plan` surface before execution, timing, arbitrary filelists, allocation, and broad native claims.
...
Review the direct command-path sidecar stage-plan materialization boundary. The definition requires validated direct fixture output to bridge through the reviewed adapter payload and existing plan-resolution helper before any `sidecar_stage_plan` materialization or reference, while still adding no execution, timing, arbitrary filelist inference, automatic allocation, or runtime/ABI change.
...
- selected next gate: `review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate`
...
- current next task: `review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `875`
...
- use `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` as the source artifact
- decide whether the direct fixture must bridge through the reviewed adapter payload and existing plan-resolution helper
- verify explicit sidecar context normalization, including `template_or_target_registry_entry`
