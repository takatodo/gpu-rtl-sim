...
1. **Native parser-to-sidecar handoff-contract boundary definition**: Define the minimal ready plan-resolution fields that may feed a future `sidecar_handoff_contract`, plus not-ready/unsupported rejection behavior, before any broader native parser support, upstream landing, sidecar execution, timing, or automatic allocation claim.
...
- selected next gate: `define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate`
...
- current next task: `define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate`
...
- use `config/scaling_gates/review_verilator_native_option_parser_sidecar_plan_resolution_status_hardening_implementation_gate.json` as the source artifact
- define the minimal ready plan-resolution fields that may feed a later `sidecar_handoff_contract`
- require `status: ready_for_verilator_option_shim` and `plan_resolution_readiness.ready_for_direct_verilator_option: true` before any handoff-contract production is even eligible
- define rejection or fallback behavior for `not_ready_for_verilator_option_shim` and `unsupported_for_stage_plan`
