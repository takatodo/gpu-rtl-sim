...
1. **Native parser-to-sidecar handoff-contract boundary review**: Review the ready-only metadata boundary that may feed a future `sidecar_handoff_contract`, plus not-ready/unsupported rejection behavior, before any broader native parser support, upstream landing, sidecar execution, timing, or automatic allocation claim.
...
- selected next gate: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate`
...
- current next task: `review_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate`
...
- use `config/scaling_gates/define_verilator_native_option_parser_sidecar_plan_resolution_handoff_contract_boundary_gate.json` as the source artifact
- decide whether the minimal ready plan-resolution fields are sufficient for future `sidecar_handoff_contract` metadata
- decide whether `status: ready_for_verilator_option_shim`, `plan_resolution_readiness.ready_for_direct_verilator_option: true`, and matching parser/stage shape are the right preconditions
- decide whether rejection behavior for `not_ready_for_verilator_option_shim` and `unsupported_for_stage_plan` is fail-closed enough
- keep command synthesis, operator plan, execution, timing, runtime/ABI, arbitrary filelist expansion, and automatic allocation out of scope
