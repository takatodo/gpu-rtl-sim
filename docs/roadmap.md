...
1. **Native parser direct command path**: Review the scoped launcher-invocation fixture that materializes the existing sidecar launcher argv from reviewed metadata.
...
- selected next gate: `review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_implementation_gate`
...
- current next task: `review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_implementation_gate`
...
- use `config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_gate.json` as the source artifact
- confirm the scoped fixture validates parser payload, bridge metadata, source review gate, and explicit sidecar context before materializing launcher argv
- confirm the structured status distinguishes bridge failure, launcher invocation construction failure, sidecar stage failure, compare failure, and timing-without-measurement
- confirm launcher argv materialization uses existing importable helpers, not shell-string guessing
