...
1. **Native parser direct command path**: Review the non-executing fixture contract for how an eventual native Verilator option invocation enters the reviewed sidecar command path, with execution, timing, arbitrary filelists, allocation, and broad native claims still deferred.
...
Review the direct native-Verilator command-path fixture contract after defining it. The review must decide whether the future importable helper contract, accepted `64x1` expanded spelling, explicit sidecar context, reference-only sidecar plan boundary, and fail-closed cases are narrow enough before any implementation, execution, timing, arbitrary filelist inference, automatic allocation, or runtime/ABI change.
...
- selected next gate: `review_verilator_native_option_parser_direct_command_path_fixture_gate`
...
- current next task: `review_verilator_native_option_parser_direct_command_path_fixture_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `867`
...
- use `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_fixture_gate.json` as the source artifact
- verify the fixture remains an importable helper contract, not an implementation or public CLI
- verify the accepted `64x1` expanded spelling preserves ordinary Verilator build inputs without source closure inference
- verify explicit sidecar context is required before any sidecar plan boundary can be referenced
- verify the output sidecar plan boundary is a structured reference only, not command-execution authority
- verify fail-closed cases cover missing shape halves, mixed compact/expanded spelling, non-positive counts, compact-only spelling, missing sidecar context, source-closure overreach, and prepopulated sidecar-owned fields
