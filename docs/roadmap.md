...
1. **Native parser direct command path**: Define how the validated direct command-path fixture can materialize or reference the reviewed `sidecar_stage_plan` surface before execution, timing, arbitrary filelists, allocation, and broad native claims.
...
Define the direct command-path sidecar stage-plan materialization boundary. The validation helper now requires exact parser-stub payload identity/build-input fields before returning reference-only sidecar plan metadata, so the next boundary must decide when that validated payload plus explicit sidecar context may call or reference the reviewed `sidecar_stage_plan` surface while still adding no execution, timing, arbitrary filelist inference, automatic allocation, or runtime/ABI change.
...
- selected next gate: `define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate`
...
- current next task: `define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate`
...
- tracked gate JSON records under `records/scaling_gates/`: `874`
...
- use `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_fixture_payload_validation_hardening_implementation_gate.json` as the source artifact
- define whether the validated direct parser payload may call the existing sidecar plan-resolution helper or only reference it
- preserve explicit sidecar context, target, mode, template or registry entry, and source gate or manifest references before any sidecar stage-plan materialization
- define ready/not-ready status mapping from `sidecar_stage_plan` outputs reached through the direct command-path fixture
- keep execution, compare, timing, runtime/ABI, arbitrary filelists, and automatic allocation outside this definition gate
