...
1. **Native parser direct command path**: Review the scoped sidecar-launcher run record and keep the `sidecar_launcher_bridge_failure` boundary distinct from the successful separate sidecar prerequisite.
...
- selected next gate: `review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_run_gate`
...
- current next task: `review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_run_gate`
...
- use `config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_gate.json` as the source artifact
- accept or reject the recorded `sidecar_launcher_bridge_failure` classification
- verify that separate `pulp_ita_mha 64x1` sidecar coverage-output equivalence is prerequisite evidence, not native-path launcher authority
- preserve the `pulp_ita_mha` / `64x1` scope until a later reviewed implementation broadens it
- record that native-path launcher invocation, sidecar stage completion, and compare remain false in this run
