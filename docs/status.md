...
Persistent resident state ABI repeat-median gate state: `config/scaling_gates/persistent_resident_state_abi_repeat_median_measurement_gate.json` defines `python3 src/tools/run_results_reproduction.py --persistent-resident-state-abi 16x64 --persistent-resident-state-abi-phases 4 --persistent-resident-state-abi-repeat-median 3`. The gate reuses the existing persistent resident ABI measurement path, writes `reports/persistent_resident_state_abi_repeat_median_summary.json`, and forbids runtime/ABI changes or new workload claims.
