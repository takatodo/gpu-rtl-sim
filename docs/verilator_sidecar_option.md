...
The current native-parser path is still overlay-only. The first repo-owned source-patch descriptor and patch file live under `overlays/verilator/patches/`, pinned to upstream Verilator `v5.048` at peeled commit `d0aa828c217410fffc73d92077b6f4f54830357c`. The patch touches only upstream `src/V3Options.h` and `src/V3Options.cpp`, adding undocumented parse/store/validate-only `--sim-accel`, `--sim-accel-states`, and `--sim-accel-steps` handling. The descriptor validates as JSON and the patch passes `git apply --check` against the pinned checkout; this still does not claim Verilator build success, native parser support, execution, measurement, runtime handoff, or arbitrary RTL/filelist support.
