...
`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` defines that boundary. A future implementation must bridge the validated direct fixture output through the reviewed parser-to-sidecar adapter payload and existing plan-resolution helper, normalize `template_or_target_registry_entry` explicitly, and preserve ready/not-ready/unsupported status mapping without execution. The next task is review; this still adds no public CLI, native option support, direct command execution, sidecar stage execution, compare execution, timing, allocation, runtime/ABI change, arbitrary filelists, or raw full-state equality.
