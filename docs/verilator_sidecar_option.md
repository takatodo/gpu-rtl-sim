...
`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` defines that boundary, and `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` accepts only the non-executing route through the reviewed parser-to-sidecar adapter payload and existing plan-resolution helper. The next task is an importable fixture implementation for that bridge; this still adds no public CLI, native option support, direct command execution, sidecar stage execution, compare execution, timing, allocation, runtime/ABI change, arbitrary filelists, or raw full-state equality.
