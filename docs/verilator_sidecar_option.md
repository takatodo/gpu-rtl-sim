...
`config/scaling_gates/run_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_gate.json` records that scoped run for `pulp_ita_mha 64x1` through `python3 src/tools/run_hybrid_template.py config/slice_launch_templates/pulp_ita_mha.json --shape 64x1` after validating ready direct materialization metadata, parser schedule constraints, and the reviewed seven-stage order. The generated compare evidence passes `coverage_output_equivalence` with mismatch count `0` across `64` state pairs, while raw full-state match remains false. `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_execution_run_gate.json` accepts only that scoped sidecar build/run/compare result and advances to defining a true native Verilator invocation boundary. The accepted run still does not prove native option support, direct Verilator command execution, timing, arbitrary filelist support, automatic allocation, runtime/ABI change, or raw full-state equality.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_boundary_gate.json` defines that boundary. The minimum future native invocation is a real Verilator-facing command carrying expanded `--sim-accel sidecar-gpu --sim-accel-states <N> --sim-accel-steps <S>` values, but this command alone is not enough to execute sidecar stages: the adapter still needs reviewed sidecar context such as target, mode, template or registry entry, and source gate or manifest. Wrapper execution, direct fixture metadata, materialized stage-plan metadata, and generated command text remain preview or test inputs, not native runtime authority.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_boundary_gate.json` accepts that boundary only for a later non-executing native-invocation fixture. The review keeps the native command alone, wrapper execution, direct fixture metadata, materialized stage-plan metadata, and generated command text out of execution authority. It permits only fixture implementation next; it still adds no native Verilator option support, direct Verilator command execution, sidecar execution, compare execution, timing, arbitrary filelist support, automatic allocation, runtime/ABI change, or raw full-state equality.

`config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_native_invocation_fixture_gate.json` implements that fixture in `src/tools/verilator_native_option_parser_direct_native_invocation_fixture.py`. It accepts a real Verilator-facing argv only when explicit sidecar context is also present, and it accepts reviewed parser payloads as metadata-only input. The fixture keeps `verilator_process_invoked`, wrapper execution, generated command authority, materialized stage-plan authority, sidecar execution, timing, filelist expansion, automatic allocation, runtime/ABI change, and raw full-state equality claims false; the next gate is implementation review.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_fixture_implementation_gate.json` accepts only that metadata validation helper. `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_execution_boundary_gate.json` then defines the next boundary as a future real Verilator process parse plus explicit sidecar context and reviewed stage authority, and `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_execution_boundary_gate.json` accepts that boundary only for a scoped future run. `config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_execution_gate.json` records the first local process parse attempt: Verilator 5.044 rejects `--sim-accel` as an invalid option on an isolated smoke RTL probe. `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_execution_run_gate.json` accepts only that local process-parse failure and selects a patched-binary selection definition next. `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_patched_binary_selection_gate.json` selects the rebuilt parser-hardening patched Verilator binary as the next parser-only retry candidate while keeping the artifact non-canonical, and `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_patched_binary_selection_gate.json` accepts that candidate only for a parser-only retry that must record exact artifact presence or an equivalent rebuild. `config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_patched_binary_retry_gate.json` then records the patched binary accepting the expanded `--sim-accel` spelling in parser-only smoke mode. This is still not sidecar execution, compare, coverage-output equivalence, timing, arbitrary filelist support, automatic allocation, runtime/ABI change, or raw-state evidence.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_patched_binary_retry_run_gate.json` accepts only that parser-only smoke success and keeps the generated patched binary non-canonical. The next boundary is sidecar authority definition: a native parser success may not execute sidecar build/run/compare until reviewed sidecar context, stage authority, artifact provenance, and compare-output words are defined.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_sidecar_authority_boundary_gate.json` defines that authority boundary without executing it. Parser success alone remains non-authoritative; a future native sidecar run must first show explicit sidecar context, reviewed stage authority, generated-artifact provenance or rebuild policy, separated failure classes, and an explicit `coverage_output_equivalence` output-word policy.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_sidecar_authority_boundary_gate.json` accepts only that authority definition and selects a first scoped native sidecar execution boundary definition next. This is still non-executing: it does not prove sidecar build/run/compare, coverage-output equivalence, timing, arbitrary filelist support, automatic allocation, runtime/ABI behavior, or raw full-state equality.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_first_scoped_sidecar_execution_boundary_gate.json` defines that first scoped boundary without executing it. The only selected future attempt is `pulp_ita_mha` with `config/slice_launch_templates/pulp_ita_mha.json` at `64x1`; it requires the reviewed patched binary or equivalent rebuild, explicit sidecar context, the seven reviewed sidecar stages, generated evidence only under `reports/` and `artifacts/`, and `coverage_output_equivalence` output-word authority.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_first_scoped_sidecar_execution_boundary_gate.json` accepts only that future-run boundary. It moves the current task to a scoped run gate, while still making no sidecar execution, compare, coverage-output equivalence, timing, arbitrary filelist, automatic allocation, runtime/ABI, or raw-state claim.

`config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_first_scoped_sidecar_execution_gate.json` records the first scoped run. The patched Verilator binary accepts the expanded native option spelling, and the reviewed `pulp_ita_mha` / `64x1` sidecar stage path passes `coverage_output_equivalence` with mismatch count `0`; the record still does not claim that the Verilator command directly launched sidecar stages.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_first_scoped_sidecar_execution_run_gate.json` accepts only that separated evidence. The next boundary is direct launch authority: defining when the Verilator-facing command itself may be treated as sidecar stage execution authority, without adding timing, arbitrary filelist support, automatic allocation, runtime/ABI changes, raw-state equality, or production-throughput claims.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_sidecar_launch_boundary_gate.json` defines that authority chain for review. A future run must connect one Verilator-facing process parse, explicit `pulp_ita_mha 64x1` sidecar context, reviewed stage authority, sidecar launch, and compare; this definition still does not prove direct sidecar execution.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_sidecar_launch_boundary_gate.json` accepts only that future-run chain and advances to a scoped run gate. The review itself still does not prove direct sidecar execution.

`config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_direct_sidecar_launch_gate.json` records the scoped run outcome. The patched Verilator process parses the expanded sidecar spelling, and the separate reviewed `pulp_ita_mha 64x1` sidecar path still passes `coverage_output_equivalence`, but the native path does not launch sidecar stages. The failure class is `direct_launch_handoff_failure`, so direct Verilator sidecar execution remains unproven.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_sidecar_launch_run_gate.json` accepts that outcome as a truthful failure record and selects the direct-launch handoff implementation boundary. The next boundary must define how native parser output, explicit sidecar context, and reviewed stage authority can reach the sidecar stage launcher without treating generated command text or stage-plan metadata as authority by themselves.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_implementation_boundary_gate.json` defines that implementation boundary for review. The only future implementation surface is the handoff from validated native parser payload plus explicit `pulp_ita_mha 64x1` sidecar context and reviewed stage authority into the existing sidecar stage launcher. This definition still does not prove direct sidecar execution, native-path compare, timing, arbitrary filelist support, automatic allocation, runtime/ABI changes, raw-state equality, or production throughput.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_implementation_boundary_gate.json` accepts only that narrow implementation boundary and selects a non-executing handoff fixture next. The fixture may model readiness to enter the existing sidecar stage launcher, but it must still not execute sidecar stages, compare outputs, measure timing, broaden filelist/RTL support, change runtime/ABI behavior, or treat generated reports/artifacts as authority.

`config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_fixture_gate.json` implements that non-executing fixture as `define_direct_launch_handoff_fixture`. It records `handoff_ready` metadata and preserves failure classes, while `sidecar_launch_reached_from_native_path` and `coverage_output_compare_reached_from_native_path` remain false.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_bridge_boundary_gate.json` accepts the next bridge boundary only for a non-executing fixture implementation. The fixture may connect validated native parser payload and reviewed handoff fixture metadata to an existing sidecar launcher reference, but it must still not execute sidecar stages, run compare, measure timing, broaden filelist/RTL support, change runtime/ABI behavior, or claim direct Verilator sidecar execution.

`config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_bridge_fixture_gate.json` records that fixture as `define_sidecar_launcher_bridge_fixture`. It validates reviewed handoff metadata, preserves `sidecar_launcher_bridge_failure`, and marks `src/tools/run_hybrid_template.py` as reference-only. It still does not call the launcher or make native-path compare, timing, runtime/ABI, arbitrary filelist, automatic allocation, or direct sidecar-execution claims.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_bridge_fixture_implementation_gate.json` accepts that bridge fixture only as metadata. The next step is defining a scoped sidecar-launcher run boundary; this still does not invoke the launcher, execute sidecar stages, compare native-path outputs, measure timing, broaden filelist/RTL support, allocate GPUs automatically, or change runtime/ABI behavior.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_run_boundary_gate.json` defines that future scoped run boundary. A later reviewed run must start from reviewed bridge metadata, stay on `pulp_ita_mha 64x1`, invoke the existing launcher from the native path, and reach `coverage_output_equivalence`; the definition itself still does not provide launcher invocation, native-path compare, timing, runtime/ABI, arbitrary filelist, automatic allocation, or direct sidecar-execution evidence.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_run_boundary_gate.json` accepts that boundary only as a future-run definition. The next run may attempt launcher invocation from reviewed bridge metadata, but the review itself still does not prove launcher invocation, sidecar execution, native-path compare, timing, runtime/ABI, arbitrary filelist support, automatic allocation, or direct Verilator sidecar execution.

`config/scaling_gates/run_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_gate.json` records that scoped attempt. It accepts the patched native-option spelling and validates bridge fixture readiness, and the separate `run_hybrid_template.py` prerequisite still passes `coverage_output_equivalence` for `pulp_ita_mha 64x1`, but the native path remains `sidecar_launcher_bridge_failure` because the bridge fixture does not invoke the launcher. This record still does not prove native-path launcher invocation, native-path compare, timing, runtime/ABI, arbitrary filelist support, automatic allocation, or direct Verilator sidecar execution.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_run_gate.json` accepts that failure record as honest evidence and selects a launcher-invocation boundary definition next. The separate sidecar result remains prerequisite evidence only; native-path launcher invocation, native-path compare, timing, runtime/ABI, arbitrary filelist support, automatic allocation, and direct Verilator sidecar execution remain non-claims.

`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_boundary_gate.json` defines that launcher-invocation implementation boundary. It may connect reviewed parser payload, reviewed bridge metadata, explicit `pulp_ita_mha 64x1` sidecar context, and the existing `src/tools/run_hybrid_template.py` launcher after review, but the definition itself still does not invoke the launcher or prove native-path compare, timing, runtime/ABI, arbitrary filelist support, automatic allocation, or direct Verilator sidecar execution.

`config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_boundary_gate.json` accepts that boundary only for a scoped fixture implementation. The next fixture may materialize launcher invocation from reviewed metadata through existing importable helpers, but this review still does not prove native-path launcher invocation, native-path compare, timing, runtime/ABI, arbitrary filelist support, automatic allocation, or direct Verilator sidecar execution.

`config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_fixture_gate.json` implements that scoped fixture as `define_sidecar_launcher_invocation_fixture`. It validates reviewed parser and bridge metadata, requires the accepted sidecar-launcher run review gate, and materializes `run_hybrid_template.py` argv through an existing helper. The fixture still does not start the launcher, execute sidecar stages, reach native-path compare, measure timing, change runtime/ABI behavior, infer arbitrary filelists, or prove direct Verilator sidecar execution.
...
Current native direct-command work is narrower than the shim preview surface. `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_bridge_boundary_gate.json` defines only a future bridge from validated native parser metadata and reviewed handoff fixture readiness into the existing `src/tools/run_hybrid_template.py` launcher for `pulp_ita_mha 64x1`. It is definition-only: no direct Verilator sidecar execution, native-path compare, timing, arbitrary filelist support, automatic allocation, runtime/ABI change, raw-state equality, or production-throughput evidence is claimed.
