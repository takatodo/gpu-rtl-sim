...
Preflight, summary, and operator-plan JSON include `operator_entrypoint` to record whether the invocation used `--sidecar-gpu`, explicit `--sim-accel`, or the ordinary target/shape surface. It also records `shape_source` as `shape`, `sim_accel_shape`, or `sim_accel_states_steps` so compact and expanded Verilator-style spellings stay auditable after normalization. This is wrapper invocation metadata only, not execution or correctness evidence.
