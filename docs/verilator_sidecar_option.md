...
`config/scaling_gates/define_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` defines that boundary, and `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_boundary_gate.json` accepts only the non-executing route through the reviewed parser-to-sidecar adapter payload and existing plan-resolution helper. `config/scaling_gates/implement_verilator_native_option_parser_direct_command_path_sidecar_stage_plan_materialization_fixture_gate.json` implements that bridge in `src/tools/verilator_native_option_parser_direct_stage_plan_materialization.py`; the next task is review. This still adds no public CLI, native option support, direct command execution, sidecar stage execution, compare execution, timing, allocation, runtime/ABI change, arbitrary filelists, or raw full-state equality.
