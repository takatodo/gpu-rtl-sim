...
int main(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (arg == "--backpressure") backpressure = true;
            ...
            std::fprintf(stderr, "usage: %s [--backpressure] [--dump-state PATH]\n", argv[0]);
            return 2;
        }
    }
    ...
    std::printf("RESULT done=%u oracle_violation=%u d_data=%08x d_error=%u intg_error=%u backpressure=%u\n",
                top.done_o, top.oracle_violation_o, top.observed_d_data_o,
                top.observed_d_error_o, top.observed_intg_error_o, backpressure ? 1 : 0);
