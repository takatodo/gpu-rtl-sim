...
Archive note: this is Codex-facing historical result memory, not current source
of truth. It may preserve older evidence wording and should not override
`docs/results.md`, `config/selection.json`, `docs/status.md`,
`docs/roadmap.md`, or `README.md`.
...
Local cleanup: large or stale trees under `artifacts/` (for example `mobile_vit/venv`, unused `*_obj_dir`) may be deleted to reclaim disk when you can rerun documented commands; see README **Local disk hygiene**.
...
Verilator native direct-command launcher invocation review: `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_run_gate.json` accepts only that structured launcher invocation plus scoped `coverage_output_equivalence` result. The next open boundary is a full native Verilator process-to-launcher definition; direct Verilator sidecar execution remains unproven.

Verilator native full process-to-launcher boundary: `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_full_native_process_to_launcher_boundary_gate.json` defines a non-executing boundary that keeps process parse, launcher argv materialization, launcher start, sidecar stages, and compare as separate claims. `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_full_native_process_to_launcher_boundary_gate.json` accepts that boundary only as review evidence, `config/scaling_gates/define_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_full_native_process_to_launcher_run_boundary_gate.json` defines the scoped run boundary with explicit `runner_kind` separation for `patched_verilator_binary` versus `reviewed_process_shim`, and `config/scaling_gates/review_verilator_native_option_parser_direct_command_path_native_invocation_direct_launch_handoff_sidecar_launcher_invocation_full_native_process_to_launcher_run_boundary_gate.json` accepts that run boundary before execution. This still adds no execution, timing, broad native option support, automatic allocation, runtime/ABI change, arbitrary filelist support, raw-state equality, or production-serving claim.
