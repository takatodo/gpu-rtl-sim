...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_lc_sync.

module prim_lc_sync_gpu_cov_tb (
  ...
  localparam logic [LcTxWidth-1:0] ResetValue = LcOffRaw;
  ...
  logic rst_ni;
  lc_tx_t lc_en_i;
  lc_tx_t [NumCopies-1:0] lc_en_o;
  logic [LcTxWidth-1:0] lc_raw_q;
  logic [LcTxWidth-1:0] last_lc_raw_q;
  logic [LcTxWidth-1:0] expected_stage1_q;
  logic [LcTxWidth-1:0] expected_stage2_q;
  logic [LcTxWidth-1:0] last_expected_stage2_q;
  logic [LcTxWidth-1:0] output0_w;
  logic [LcTxWidth-1:0] output1_w;
  logic [LcTxWidth-1:0] last_output0_q;
  logic [LcTxWidth-1:0] last_output1_q;
  logic [1:0] phase_w;
  ...
  logic [31:0] on_input_count_q;
  logic [31:0] off_input_count_q;
  ...
  assign lc_en_i = lc_tx_t'(lc_raw_q);
  assign output0_w = lc_en_o[0];
  assign output1_w = lc_en_o[1];
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h1c5c_5001;
      lc_raw_q <= ResetValue;
      last_lc_raw_q <= ResetValue;
      expected_stage1_q <= ResetValue;
      expected_stage2_q <= ResetValue;
      last_expected_stage2_q <= ResetValue;
      last_output0_q <= ResetValue;
      last_output1_q <= ResetValue;
      post_reset_seen_q <= 1'b0;
      input_transition_count_q <= 32'd0;
      on_input_count_q <= 32'd0;
      off_input_count_q <= 32'd0;
      ...
      signature_q <= cfg_seed_i ^ 32'h1c5c_5eed;
      ...
      last_lc_raw_q <= lc_raw_q;
      last_expected_stage2_q <= expected_stage2_q;
      last_output0_q <= output0_w;
      last_output1_q <= output1_w;
      expected_stage1_q <= lc_raw_q;
      ...
      if (lc_raw_q == LcOnRaw) begin
        on_input_count_q <= on_input_count_q + 32'd1;
        seen_q[3] <= 1'b1;
      end else if (lc_raw_q == LcOffRaw) begin
        off_input_count_q <= off_input_count_q + 32'd1;
        seen_q[4] <= 1'b1;
      end else begin
      ...
      if (output0_w != last_output0_q) begin
        output0_change_count_q <= output0_change_count_q + 32'd1;
        seen_q[6] <= 1'b1;
      end
      if (output1_w != last_output1_q) begin
        output1_change_count_q <= output1_change_count_q + 32'd1;
        seen_q[7] <= 1'b1;
      end
      if (post_reset_seen_q && output0_w == expected_stage2_q &&
          output1_w == expected_stage2_q) begin
      ...
      if (output0_w != output1_w) begin
        copy_mismatch_count_q <= copy_mismatch_count_q + 32'd1;
        seen_q[10] <= 1'b1;
      end
      if (output0_w == ResetValue && output1_w == ResetValue) begin
      ...
      signature_q <= {signature_q[27:0], signature_q[31:28]} ^
                     {20'd0, lc_raw_q, output0_w, output1_w} ^
  ...
  assign progress_signature_o = signature_q ^ {28'd0, lc_raw_q} ^
                                {24'd0, output0_w, output1_w};
  assign cfg_signature_o = progress_signature_o ^ cfg_seed_i ^ 32'h1c5c_5eed;

  assign host_req_accepted_o = on_input_count_q;
  assign device_req_accepted_o = off_input_count_q;
  ...
  assign real_toggle_subset_word1_o = on_input_count_q;
  assign real_toggle_subset_word2_o = off_input_count_q;
  ...
  assign real_toggle_subset_word14_o = {20'd0, lc_raw_q, expected_stage2_q, output0_w};
  ...
  assign focused_wave_word0_o = {28'd0, lc_raw_q};
  assign focused_wave_word1_o = {28'd0, expected_stage2_q};
  assign focused_wave_word2_o = {28'd0, output0_w};
  assign focused_wave_word3_o = {28'd0, output1_w};
  ...
  assign oracle_semantic_family_acked_o = {28'd0, expected_stage2_q};
  assign oracle_semantic_case_seen_o = {28'd0, lc_raw_q};
  assign oracle_semantic_case_acked_o = {24'd0, output0_w, output1_w};
...
endmodule : prim_lc_sync_gpu_cov_tb
