...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_mubi8_sender.

module prim_mubi8_sender_gpu_cov_tb (
  ...
  logic [7:0] expected_stage_q;
  logic [7:0] last_expected_stage_q;
  logic [7:0] last_output_q;
  logic [1:0] phase_w;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      expected_stage_q <= MuBi8Width'(ResetValue);
      last_expected_stage_q <= MuBi8Width'(ResetValue);
      last_output_q <= MuBi8Width'(ResetValue);
      ...
      last_output_q <= MuBi8Width'(mubi_o);
      ...
      if (MuBi8Width'(mubi_o) != last_output_q) begin
        output_change_count_q <= output_change_count_q + 32'd1;
        seen_q[6] <= 1'b1;
      end
      if (post_reset_seen_q && MuBi8Width'(mubi_o) == expected_stage_q) begin
      ...
      if (MuBi8Width'(mubi_o) == MuBi8Width'(ResetValue)) begin
      ...
      signature_q <= {signature_q[27:0], signature_q[31:28]} ^
                     {16'd0, mubi_raw_q, MuBi8Width'(mubi_o)} ^
  ...
  assign real_toggle_subset_word12_o = {8'd0, mubi_raw_q, expected_stage_q,
                                        MuBi8Width'(mubi_o)};
  assign real_toggle_subset_word13_o = {8'd0, last_mubi_raw_q, last_expected_stage_q,
                                        last_output_q};
  ...
  assign focused_wave_word1_o = {24'd0, expected_stage_q};
  assign focused_wave_word2_o = {24'd0, MuBi8Width'(mubi_o)};
  assign focused_wave_word3_o = {24'd0, last_output_q};
  ...
  assign oracle_semantic_family_acked_o = {24'd0, expected_stage_q};
  assign oracle_semantic_case_seen_o = {24'd0, mubi_raw_q};
  assign oracle_semantic_case_acked_o = {24'd0, MuBi8Width'(mubi_o)};
...
endmodule : prim_mubi8_sender_gpu_cov_tb
