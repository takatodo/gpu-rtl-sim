...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_mubi8_sync.

module prim_mubi8_sync_gpu_cov_tb (
  ...
  localparam mubi8_t ResetValue = MuBi8False;
  ...
  logic rst_ni;
  mubi8_t mubi_i;
  mubi8_t [NumCopies-1:0] mubi_o;
  logic [7:0] mubi_raw_q;
  logic [7:0] last_mubi_raw_q;
  logic [7:0] expected_stage1_q;
  logic [7:0] expected_stage2_q;
  logic [7:0] last_expected_stage2_q;
  logic [7:0] last_output0_q;
  logic [7:0] last_output1_q;
  logic [1:0] phase_w;
  ...
  assign mubi_i = mubi8_t'(mubi_raw_q);
  ...
  function automatic logic [7:0] choose_mubi_raw(input logic [31:0] state);
    logic [7:0] candidate;
    if (pct_hit(state ^ cfg_req_family_i, cfg_req_valid_pct_i)) begin
      return MuBi8Width'(MuBi8True);
    end
    if (pct_hit(state ^ cfg_rsp_family_i, cfg_rsp_valid_pct_i)) begin
      return MuBi8Width'(MuBi8False);
    end
    candidate = state[7:0] ^ cfg_req_data_mode_i[7:0] ^ cfg_rsp_data_mode_i[7:0];
    if (candidate == MuBi8Width'(MuBi8True)) begin
      return 8'h3c;
    end
    if (candidate == MuBi8Width'(MuBi8False)) begin
      return 8'hc3;
    end
    return candidate;
  endfunction
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h8b1c_5001;
      mubi_raw_q <= MuBi8Width'(ResetValue);
      last_mubi_raw_q <= MuBi8Width'(ResetValue);
      expected_stage1_q <= MuBi8Width'(ResetValue);
      expected_stage2_q <= MuBi8Width'(ResetValue);
      last_expected_stage2_q <= MuBi8Width'(ResetValue);
      last_output0_q <= MuBi8Width'(ResetValue);
      last_output1_q <= MuBi8Width'(ResetValue);
      ...
      signature_q <= cfg_seed_i ^ 32'h8b1c_5eed;
      ...
      last_output0_q <= MuBi8Width'(mubi_o[0]);
      last_output1_q <= MuBi8Width'(mubi_o[1]);
      ...
      if (phase_w == TrafficPhase) begin
        ...
        mubi_raw_q <= (drain_cycle_q[0]) ? MuBi8Width'(MuBi8True) : MuBi8Width'(MuBi8False);
      ...
      if (mubi_raw_q == MuBi8Width'(MuBi8True)) begin
        true_input_count_q <= true_input_count_q + 32'd1;
        seen_q[3] <= 1'b1;
      end else if (mubi_raw_q == MuBi8Width'(MuBi8False)) begin
      ...
      if (MuBi8Width'(mubi_o[0]) != last_output0_q) begin
        output0_change_count_q <= output0_change_count_q + 32'd1;
        seen_q[6] <= 1'b1;
      end
      if (MuBi8Width'(mubi_o[1]) != last_output1_q) begin
        output1_change_count_q <= output1_change_count_q + 32'd1;
        seen_q[7] <= 1'b1;
      end
      if (post_reset_seen_q && MuBi8Width'(mubi_o[0]) == expected_stage2_q &&
          MuBi8Width'(mubi_o[1]) == expected_stage2_q) begin
      ...
      if (MuBi8Width'(mubi_o[0]) != MuBi8Width'(mubi_o[1])) begin
        copy_mismatch_count_q <= copy_mismatch_count_q + 32'd1;
        seen_q[10] <= 1'b1;
      end
      if (MuBi8Width'(mubi_o[0]) == MuBi8Width'(ResetValue) &&
          MuBi8Width'(mubi_o[1]) == MuBi8Width'(ResetValue)) begin
      ...
      signature_q <= {signature_q[27:0], signature_q[31:28]} ^
                     {8'd0, mubi_raw_q, MuBi8Width'(mubi_o[0]), MuBi8Width'(mubi_o[1])} ^
  ...
  assign progress_signature_o = signature_q ^ {24'd0, mubi_raw_q} ^
                                {16'd0, MuBi8Width'(mubi_o[0]), MuBi8Width'(mubi_o[1])};
  assign cfg_signature_o = progress_signature_o ^ cfg_seed_i ^ 32'h8b1c_5eed;
  ...
  assign real_toggle_subset_word14_o = {8'd0, mubi_raw_q, expected_stage2_q,
                                        MuBi8Width'(mubi_o[0])};
  ...
  assign focused_wave_word0_o = {24'd0, mubi_raw_q};
  assign focused_wave_word1_o = {24'd0, expected_stage2_q};
  assign focused_wave_word2_o = {24'd0, MuBi8Width'(mubi_o[0])};
  assign focused_wave_word3_o = {24'd0, MuBi8Width'(mubi_o[1])};
  ...
  assign oracle_semantic_family_acked_o = {24'd0, expected_stage2_q};
  assign oracle_semantic_case_seen_o = {24'd0, mubi_raw_q};
  assign oracle_semantic_case_acked_o = {16'd0, MuBi8Width'(mubi_o[0]), MuBi8Width'(mubi_o[1])};
...
endmodule : prim_mubi8_sync_gpu_cov_tb
