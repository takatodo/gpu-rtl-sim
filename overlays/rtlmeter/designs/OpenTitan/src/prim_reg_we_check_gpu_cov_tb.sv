...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_reg_we_check.

module prim_reg_we_check_gpu_cov_tb (
  ...
  localparam int unsigned OneHotWidth = 4;
  ...
  function automatic int unsigned count_ones4(input logic [OneHotWidth-1:0] bits);
    int unsigned count;
    count = 0;
  ...
  function automatic logic [OneHotWidth-1:0] onehot_for_index(input logic [1:0] index);
  ...
  function automatic logic [OneHotWidth-1:0] choose_oh(
    ...
    index = state[1:0];
    base = onehot_for_index(index);
    alternate = onehot_for_index(index + 2'd1);
    noise = state[OneHotWidth-1:0];
    unique case (case_id)
      3'd0: return base;
      3'd1: return '0;
      3'd2: return base | alternate;
      3'd3: return noise;
      3'd4: return '0;
      3'd5: return base;
      3'd6: return base | onehot_for_index(index + 2'd2);
  ...
  assign next_oh_w = choose_oh(rand_q ^ cfg_req_data_hi_xor_i, next_case_w);
  assign next_en_w = choose_en(rand_q ^ cfg_rsp_error_pct_i, next_case_w);
  assign any_oh_w = |oh_i;
  assign onehot0_err_w = count_ones4(oh_i) > 1;
  assign enable_err_w = !en_i && any_oh_w;
  assign expected_err_w = onehot0_err_w || enable_err_w;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h5eed_0ace;
      oh_i <= '0;
      en_i <= 1'b0;
      ...
      onehot_count_q <= 32'd0;
      multi_hot_count_q <= 32'd0;
      signature_q <= cfg_seed_i ^ 32'hc005_e112;
      ...
      if (any_oh_w && !onehot0_err_w) begin
        onehot_count_q <= onehot_count_q + 32'd1;
        seen_q[11] <= 1'b1;
      end
  ...
  assign real_toggle_subset_word7_o = zero_enabled_count_q;
  assign real_toggle_subset_word8_o = zero_disabled_count_q;
  assign real_toggle_subset_word9_o = onehot_count_q;
  assign real_toggle_subset_word10_o = multi_hot_count_q;
  assign real_toggle_subset_word11_o = {29'd0, case_w};
  ...
  assign focused_wave_word0_o = {28'd0, oh_i};
  assign focused_wave_word1_o = {27'd0, case_w, en_i, err_o};
  ...
  assign focused_wave_word5_o = onehot0_err_count_q ^ enable_err_count_q;
  ...
  assign oracle_semantic_family_acked_o = {
    ...
    onehot_count_q[7:0],
    multi_hot_count_q[7:0]
  };
  assign oracle_semantic_case_seen_o = {22'd0, case_w, next_case_w, oh_i};
...
endmodule : prim_reg_we_check_gpu_cov_tb
