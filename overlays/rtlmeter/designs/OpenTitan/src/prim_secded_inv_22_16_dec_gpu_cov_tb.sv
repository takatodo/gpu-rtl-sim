...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_secded_inv_22_16_dec.

module prim_secded_inv_22_16_dec_gpu_cov_tb (
  ...
  localparam logic [CodeWidth-1:0] InversionMask = 22'h2A0000;
  ...
  function automatic logic [5:0] syndrome_ref(input logic [CodeWidth-1:0] code);
    ...
    syn[0] = ^(unmasked & 22'h01496E);
    syn[1] = ^(unmasked & 22'h02F20B);
    syn[2] = ^(unmasked & 22'h048ED8);
    syn[3] = ^(unmasked & 22'h087714);
    syn[4] = ^(unmasked & 22'h10ACA5);
    syn[5] = ^(unmasked & 22'h2011F3);
    return syn;
  endfunction
  ...
  function automatic logic [DataWidth-1:0] decode_data_ref(input logic [CodeWidth-1:0] code);
    ...
    dec[0] = (syn == 6'h32) ^ code[0];
    dec[1] = (syn == 6'h23) ^ code[1];
    dec[2] = (syn == 6'h19) ^ code[2];
    dec[3] = (syn == 6'h07) ^ code[3];
    dec[4] = (syn == 6'h2c) ^ code[4];
    dec[5] = (syn == 6'h31) ^ code[5];
    dec[6] = (syn == 6'h25) ^ code[6];
    dec[7] = (syn == 6'h34) ^ code[7];
    dec[8] = (syn == 6'h29) ^ code[8];
    dec[9] = (syn == 6'h0e) ^ code[9];
    dec[10] = (syn == 6'h1c) ^ code[10];
    dec[11] = (syn == 6'h15) ^ code[11];
    dec[12] = (syn == 6'h2a) ^ code[12];
    dec[13] = (syn == 6'h1a) ^ code[13];
    dec[14] = (syn == 6'h0b) ^ code[14];
    dec[15] = (syn == 6'h16) ^ code[15];
    return dec;
  endfunction
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h22_16_00a5;
      data_i <= '0;
      last_data_q <= '0;
      last_data_o_q <= '0;
      ...
      signature_q <= cfg_seed_i ^ 32'h1e22_16a5;
...
endmodule : prim_secded_inv_22_16_dec_gpu_cov_tb
