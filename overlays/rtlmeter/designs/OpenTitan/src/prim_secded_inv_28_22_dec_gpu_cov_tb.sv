...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_secded_inv_28_22_dec.

module prim_secded_inv_28_22_dec_gpu_cov_tb (
  ...
  localparam logic [CodeWidth-1:0] InversionMask = 28'hA800000;
  ...
  function automatic logic [5:0] syndrome_ref(input logic [CodeWidth-1:0] code);
    ...
    syn[0] = ^(unmasked & 28'h07003FF);
    syn[1] = ^(unmasked & 28'h090FC0F);
    syn[2] = ^(unmasked & 28'h1271C71);
    syn[3] = ^(unmasked & 28'h23B6592);
    syn[4] = ^(unmasked & 28'h43DAAA4);
    syn[5] = ^(unmasked & 28'h83ED348);
    return syn;
  endfunction
  ...
  function automatic logic [DataWidth-1:0] decode_data_ref(input logic [CodeWidth-1:0] code);
    ...
    dec[0] = (syn == 6'h07) ^ code[0];
    dec[1] = (syn == 6'h0b) ^ code[1];
    dec[2] = (syn == 6'h13) ^ code[2];
    dec[3] = (syn == 6'h23) ^ code[3];
    dec[4] = (syn == 6'h0d) ^ code[4];
    dec[5] = (syn == 6'h15) ^ code[5];
    dec[6] = (syn == 6'h25) ^ code[6];
    dec[7] = (syn == 6'h19) ^ code[7];
    dec[8] = (syn == 6'h29) ^ code[8];
    dec[9] = (syn == 6'h31) ^ code[9];
    dec[10] = (syn == 6'h0e) ^ code[10];
    dec[11] = (syn == 6'h16) ^ code[11];
    dec[12] = (syn == 6'h26) ^ code[12];
    dec[13] = (syn == 6'h1a) ^ code[13];
    dec[14] = (syn == 6'h2a) ^ code[14];
    dec[15] = (syn == 6'h32) ^ code[15];
    dec[16] = (syn == 6'h1c) ^ code[16];
    dec[17] = (syn == 6'h2c) ^ code[17];
    dec[18] = (syn == 6'h34) ^ code[18];
    dec[19] = (syn == 6'h38) ^ code[19];
    dec[20] = (syn == 6'h3b) ^ code[20];
    dec[21] = (syn == 6'h3d) ^ code[21];
    return dec;
  endfunction
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h28_22_00a5;
      data_i <= '0;
      last_data_q <= '0;
      last_data_o_q <= '0;
      ...
      signature_q <= cfg_seed_i ^ 32'h1e28_22a5;
      ...
      signature_q <= {signature_q[24:0], signature_q[31:25]} ^
                     rand_q ^ {4'd0, data_i} ^ {10'd0, data_o} ^
      ...
      if (|data_i[27:18]) begin data_high_activity_count_q <= data_high_activity_count_q + 32'd1; seen_q[12] <= 1'b1; end
  ...
  assign real_toggle_subset_word15_o = {4'd0, data_i} ^ {4'd0, last_data_q};
  ...
  assign focused_wave_word0_o = {4'd0, data_i};
  ...
  assign oracle_semantic_case_seen_o = {4'd0, data_i};
...
endmodule : prim_secded_inv_28_22_dec_gpu_cov_tb
