...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_secded_inv_39_32_dec.

module prim_secded_inv_39_32_dec_gpu_cov_tb (
  ...
  function automatic logic [6:0] syndrome_ref(input logic [CodeWidth-1:0] code);
    logic [6:0] syn;
    syn[0] = ^((code ^ InversionMask) & 39'h012606BD25);
    syn[1] = ^((code ^ InversionMask) & 39'h02DEBA8050);
    syn[2] = ^((code ^ InversionMask) & 39'h04413D89AA);
    syn[3] = ^((code ^ InversionMask) & 39'h0831234ED1);
    syn[4] = ^((code ^ InversionMask) & 39'h10C2C1323B);
    syn[5] = ^((code ^ InversionMask) & 39'h202DCC624C);
    syn[6] = ^((code ^ InversionMask) & 39'h4098505586);
    return syn;
  endfunction
  ...
  function automatic logic [DataWidth-1:0] decode_data_ref(input logic [CodeWidth-1:0] code);
    ...
    dec[0] = (syn == 7'h19) ^ code[0];
    dec[1] = (syn == 7'h54) ^ code[1];
    dec[2] = (syn == 7'h61) ^ code[2];
    dec[3] = (syn == 7'h34) ^ code[3];
    dec[4] = (syn == 7'h1a) ^ code[4];
    dec[5] = (syn == 7'h15) ^ code[5];
    dec[6] = (syn == 7'h2a) ^ code[6];
    dec[7] = (syn == 7'h4c) ^ code[7];
    dec[8] = (syn == 7'h45) ^ code[8];
    dec[9] = (syn == 7'h38) ^ code[9];
    dec[10] = (syn == 7'h49) ^ code[10];
    dec[11] = (syn == 7'h0d) ^ code[11];
    dec[12] = (syn == 7'h51) ^ code[12];
    dec[13] = (syn == 7'h31) ^ code[13];
    dec[14] = (syn == 7'h68) ^ code[14];
    dec[15] = (syn == 7'h07) ^ code[15];
    dec[16] = (syn == 7'h1c) ^ code[16];
    dec[17] = (syn == 7'h0b) ^ code[17];
    dec[18] = (syn == 7'h25) ^ code[18];
    dec[19] = (syn == 7'h26) ^ code[19];
    dec[20] = (syn == 7'h46) ^ code[20];
    dec[21] = (syn == 7'h0e) ^ code[21];
    dec[22] = (syn == 7'h70) ^ code[22];
    dec[23] = (syn == 7'h32) ^ code[23];
    dec[24] = (syn == 7'h2c) ^ code[24];
    dec[25] = (syn == 7'h13) ^ code[25];
    dec[26] = (syn == 7'h23) ^ code[26];
    dec[27] = (syn == 7'h62) ^ code[27];
    dec[28] = (syn == 7'h4a) ^ code[28];
    dec[29] = (syn == 7'h29) ^ code[29];
    dec[30] = (syn == 7'h16) ^ code[30];
    dec[31] = (syn == 7'h52) ^ code[31];
    return dec;
  endfunction
...
endmodule : prim_secded_inv_39_32_dec_gpu_cov_tb
