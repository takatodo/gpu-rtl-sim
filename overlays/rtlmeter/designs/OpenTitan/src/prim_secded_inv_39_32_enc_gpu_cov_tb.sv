...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_secded_inv_39_32_enc.

module prim_secded_inv_39_32_enc_gpu_cov_tb (
  ...
  function automatic logic [CodeWidth-1:0] encode_ref(input logic [DataWidth-1:0] data);
    ...
    enc[32] = ^(enc & 39'h002606BD25);
    enc[33] = ^(enc & 39'h00DEBA8050);
    enc[34] = ^(enc & 39'h00413D89AA);
    enc[35] = ^(enc & 39'h0031234ED1);
    enc[36] = ^(enc & 39'h00C2C1323B);
    enc[37] = ^(enc & 39'h002DCC624C);
    enc[38] = ^(enc & 39'h0098505586);
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h39_32_0001;
      data_i <= '0;
      last_data_q <= '0;
      last_data_o_q <= '0;
      ...
      signature_q <= cfg_seed_i ^ 32'h1e39_3201;
...
endmodule : prim_secded_inv_39_32_enc_gpu_cov_tb
