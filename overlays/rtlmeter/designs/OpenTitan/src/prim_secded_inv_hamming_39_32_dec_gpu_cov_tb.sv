...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_secded_inv_hamming_39_32_dec.

module prim_secded_inv_hamming_39_32_dec_gpu_cov_tb (
  ...
  localparam logic [CodeWidth-1:0] InversionMask = 39'h2A00000000;
  ...
  prim_secded_inv_hamming_39_32_dec dut (
    .data_i,
    .data_o,
    .syndrome_o,
    .err_o
  ...
  function automatic logic [CodeWidth-1:0] encode_ref(input logic [DataWidth-1:0] data);
    ...
    enc[32] = ^(enc & 39'h0156AAAD5B);
    enc[33] = ^(enc & 39'h029B33366D);
    enc[34] = ^(enc & 39'h04E3C3C78E);
    enc[35] = ^(enc & 39'h0803FC07F0);
    enc[36] = ^(enc & 39'h1003FFF800);
    enc[37] = ^(enc & 39'h20FC000000);
  ...
  function automatic logic [6:0] syndrome_ref(input logic [CodeWidth-1:0] code);
    logic [CodeWidth-1:0] unmasked;
    logic [6:0] syn;
    unmasked = code ^ InversionMask;
    syn[0] = ^(unmasked & 39'h0156AAAD5B);
    syn[1] = ^(unmasked & 39'h029B33366D);
    syn[2] = ^(unmasked & 39'h04E3C3C78E);
    syn[3] = ^(unmasked & 39'h0803FC07F0);
    syn[4] = ^(unmasked & 39'h1003FFF800);
    syn[5] = ^(unmasked & 39'h20FC000000);
    syn[6] = ^(unmasked & 39'h7FFFFFFFFF);
    return syn;
  endfunction
  ...
  function automatic logic [DataWidth-1:0] decode_data_ref(input logic [CodeWidth-1:0] code);
    ...
    dec[0] = (syn == 7'h43) ^ code[0];
    dec[1] = (syn == 7'h45) ^ code[1];
    dec[2] = (syn == 7'h46) ^ code[2];
    dec[3] = (syn == 7'h47) ^ code[3];
    dec[4] = (syn == 7'h49) ^ code[4];
    dec[5] = (syn == 7'h4a) ^ code[5];
    dec[6] = (syn == 7'h4b) ^ code[6];
    dec[7] = (syn == 7'h4c) ^ code[7];
    dec[8] = (syn == 7'h4d) ^ code[8];
    dec[9] = (syn == 7'h4e) ^ code[9];
    dec[10] = (syn == 7'h4f) ^ code[10];
    dec[11] = (syn == 7'h51) ^ code[11];
    dec[12] = (syn == 7'h52) ^ code[12];
    dec[13] = (syn == 7'h53) ^ code[13];
    dec[14] = (syn == 7'h54) ^ code[14];
    dec[15] = (syn == 7'h55) ^ code[15];
    dec[16] = (syn == 7'h56) ^ code[16];
    dec[17] = (syn == 7'h57) ^ code[17];
    dec[18] = (syn == 7'h58) ^ code[18];
    dec[19] = (syn == 7'h59) ^ code[19];
    dec[20] = (syn == 7'h5a) ^ code[20];
    dec[21] = (syn == 7'h5b) ^ code[21];
    dec[22] = (syn == 7'h5c) ^ code[22];
    dec[23] = (syn == 7'h5d) ^ code[23];
    dec[24] = (syn == 7'h5e) ^ code[24];
    dec[25] = (syn == 7'h5f) ^ code[25];
    dec[26] = (syn == 7'h61) ^ code[26];
    dec[27] = (syn == 7'h62) ^ code[27];
    dec[28] = (syn == 7'h63) ^ code[28];
    dec[29] = (syn == 7'h64) ^ code[29];
    dec[30] = (syn == 7'h65) ^ code[30];
    dec[31] = (syn == 7'h66) ^ code[31];
    return dec;
  endfunction
  ...
  function automatic logic [CodeWidth-1:0] inject_error(
    ...
    unique case (selector[3:0])
      ...
      4'h7: begin out[3] = ~code[3]; out[32] = ~code[32]; end
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'h39_32_00a5;
      data_i <= '0;
      last_data_q <= '0;
      last_data_o_q <= '0;
      ...
      signature_q <= cfg_seed_i ^ 32'h1e39_32a5;
      ...
      if (|data_i[38:18]) begin data_high_activity_count_q <= data_high_activity_count_q + 32'd1; seen_q[12] <= 1'b1; end
  ...
  assign oracle_semantic_case_acked_o = data_o;
...
endmodule : prim_secded_inv_hamming_39_32_dec_gpu_cov_tb
