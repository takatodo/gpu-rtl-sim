...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_subreg_arb.

module prim_subreg_arb_gpu_cov_tb (
  ...
  logic [Width-1:0] wd_i;
  logic [Width-1:0] d_i;
  logic [Width-1:0] q_i;
  logic [Width-1:0] wr_data_o;
  logic [Width-1:0] out_o;
  logic [Width-1:0] expected_wr_data_w;
  logic expected_wr_en_w;
  ...
  assign wd_i = in0_i;
  assign d_i = in1_i;
  assign q_i = in0_i ^ in1_i ^ rand_q[Width-1:0];
  assign out_o = wr_data_o;
  assign we_i = rand_q[1] ^ cfg_rsp_valid_pct_i[0];
  assign de_i = rand_q[2] ^ cfg_device_a_ready_pct_i[0];
  assign expected_wr_en_w = we_i | de_i;
  assign expected_wr_data_w = we_i ? wd_i : d_i;
  assign expected_out_w = expected_wr_data_w;
  assign next_expected_out_w = next_in0_w;
  assign observed_match_w = (out_o == expected_out_w) && (wr_en_o == expected_wr_en_w);
  assign next_observed_match_w = next_in0_w == next_expected_out_w;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'hc5e8_0001;
      in0_i <= '0;
      in1_i <= '0;
      last_out_q <= '0;
      last_in0_q <= '0;
  ...
  assign focused_wave_word2_o = {24'd0, wr_data_o};
  assign focused_wave_word3_o = {25'd0, wr_en_o, de_i, we_i, expected_wr_en_w,
  ...
  assign oracle_semantic_case_seen_o = {14'd0, wr_en_o, de_i, in0_i, in1_i};
  assign oracle_semantic_case_acked_o = {8'd0, wr_data_o, q_i, expected_wr_data_w};
...
endmodule : prim_subreg_arb_gpu_cov_tb
