...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_subreg_ext.

module prim_subreg_ext_gpu_cov_tb (
  ...
  assign d_i = in1_i;
  assign out_o = q_o;
  assign re_i = rand_q[0] ^ cfg_req_valid_pct_i[0];
  assign we_i = rand_q[1] ^ cfg_rsp_valid_pct_i[0];
  assign expected_out_w = wd_i;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_q <= cfg_seed_i ^ 32'hc5e7_0001;
      in0_i <= '0;
      in1_i <= '0;
      last_out_q <= '0;
      last_in0_q <= '0;
  ...
  assign focused_wave_word3_o = {25'd0, qe_o, qre_o, we_i, re_i,
  ...
  assign oracle_semantic_case_seen_o = {14'd0, qe_o, qre_o, in0_i, in1_i};
...
endmodule : prim_subreg_ext_gpu_cov_tb
