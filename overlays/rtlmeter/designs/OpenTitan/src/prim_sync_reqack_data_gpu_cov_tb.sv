...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_sync_reqack_data.

module prim_sync_reqack_data_gpu_cov_tb (
  ...
  always_ff @(posedge clk_src_i) begin
    ...
    if (!rst_src_ni) begin
      ...
      if (start_req_w) begin
        src_req_q <= 1'b1;
        data_q <= mix_word(rand_src_q, src_req_count_q);
      ...
      if (src_req_q && src_ack_o) begin
        ...
        src_signature_q <= {src_signature_q[30:0], src_signature_q[31]} ^
                           data_i ^ src_ack_count_q;
        src_seen_q[1] <= 1'b1;
      end
      if (src_req_q && !src_ack_o) begin
        src_wait_count_q <= src_wait_count_q + 32'd1;
        stalled_signature_q <= stalled_signature_q ^ data_i ^ global_src_cycle_q;
        src_seen_q[2] <= 1'b1;
      end
      if (src_ack_o) begin
        src_seen_q[3] <= 1'b1;
  ...
  assign real_toggle_subset_word8_o = data_i;
  assign real_toggle_subset_word9_o = data_o;
  ...
  assign focused_wave_word2_o = data_i;
  assign focused_wave_word3_o = data_o;
