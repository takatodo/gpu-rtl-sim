...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_xilinx_ultrascale_clock_div.

module prim_xilinx_ultrascale_clock_div_gpu_cov_tb (
  ...
  logic clk_i;
  logic rst_ni;
  logic step_down_req_i;
  logic step_down_ack_o;
  logic test_en_i;
  logic clk_o;
  logic expected_clk_q;
  logic next_step_down_w;
  logic last_clk_q;
  logic last_ack_q;
  logic last_clk_o_q;
  logic [1:0] phase_w;
  ...
  prim_xilinx_ultrascale_clock_div #(
    ...
    .clk_i(clk_i),
    .rst_ni(rst_ni),
    .step_down_req_i(step_down_req_i),
    .step_down_ack_o(step_down_ack_o),
    .test_en_i(test_en_i),
    .clk_o(clk_o)
  );
  ...
  assign next_step_down_w = rand_q[3] ^ traffic_cycle_q[2] ^
                            cfg_req_address_mode_i[0] ^ cfg_rsp_delay_mode_i[0];
  assign test_en_i = 1'b0;
  assign observed_match_w = 1'b1;
  assign input_rise_w = clk_i && !last_clk_q;
  assign input_fall_w = !clk_i && last_clk_q;
  assign ack_rise_w = step_down_ack_o && !last_ack_q;
  assign ack_fall_w = !step_down_ack_o && last_ack_q;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      if (step_down_ack_o) begin
        ack_high_count_q <= ack_high_count_q + 32'd1;
        seen_q[16] <= 1'b1;
      end else begin
        ack_low_count_q <= ack_low_count_q + 32'd1;
        seen_q[17] <= 1'b1;
      end
      if (clk_o) begin
      ...
      if (ack_rise_w) begin ack_rise_count_q <= ack_rise_count_q + 32'd1; seen_q[18] <= 1'b1; end
      if (ack_fall_w) begin ack_fall_count_q <= ack_fall_count_q + 32'd1; seen_q[19] <= 1'b1; end
      ...
      if (phase_w == TrafficPhase && step_down_ack_o) begin traffic_high_count_q <= traffic_high_count_q + 32'd1; seen_q[12] <= 1'b1; end
      if (phase_w == TrafficPhase && !step_down_ack_o) begin traffic_low_count_q <= traffic_low_count_q + 32'd1; seen_q[13] <= 1'b1; end
      if (phase_w == DrainPhase && step_down_ack_o) begin drain_high_count_q <= drain_high_count_q + 32'd1; seen_q[14] <= 1'b1; end
      if (phase_w == DrainPhase && !step_down_ack_o) begin drain_low_count_q <= drain_low_count_q + 32'd1; seen_q[15] <= 1'b1; end
    end
  end
  ...
  assign real_toggle_subset_word2_o = ack_high_count_q;
  assign real_toggle_subset_word3_o = ack_low_count_q;
  assign real_toggle_subset_word4_o = input_rise_count_q;
  assign real_toggle_subset_word5_o = input_fall_count_q;
  assign real_toggle_subset_word6_o = ack_rise_count_q;
  assign real_toggle_subset_word7_o = ack_fall_count_q;
  ...
  assign focused_wave_word0_o = {31'd0, clk_i};
  assign focused_wave_word1_o = {31'd0, step_down_req_i};
  assign focused_wave_word2_o = {28'd0, output_fall_w, output_rise_w,
                                 input_fall_w, input_rise_w};
  assign focused_wave_word3_o = {28'd0, observed_match_w, clk_o, step_down_ack_o,
                                 expected_clk_q};
  ...
  assign oracle_semantic_family_acked_o = {
    ...
    ack_rise_count_q[7:0],
    ack_fall_count_q[7:0]
  };
  assign oracle_semantic_case_seen_o = {28'd0, clk_o, expected_clk_q, step_down_ack_o, clk_i};
...
endmodule : prim_xilinx_ultrascale_clock_div_gpu_cov_tb
