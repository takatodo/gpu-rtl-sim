...
//
// Minimal GPU-toggle-ready standalone coverage target for prim_xilinx_ultrascale_flop_en.

module prim_xilinx_ultrascale_flop_en_gpu_cov_tb (
  ...
  logic enable_hit_w;
  logic last_en_q;
  logic disabled_update_violation_q;
  ...
  logic [31:0] hold_count_q;
  logic [31:0] q_change_count_q;
  logic [31:0] q_hold_match_count_q;
  logic [31:0] update_applied_count_q;
  ...
  assign d_i = d_q;
  assign en_i = (phase_w == TrafficPhase) && enable_hit_w;
  ...
  always_comb begin
    ...
    data_mask_w[7] = pct_hit(~rand_q ^ cfg_rsp_error_pct_i ^ cfg_rsp_delay_mode_i,
                             cfg_rsp_error_pct_i);
    enable_hit_w = pct_hit(rand_q ^ cfg_seed_i ^ cfg_rsp_delay_max_i,
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (!rst_ni) begin
      ...
      last_q_q <= ResetValue;
      last_en_q <= 1'b0;
      disabled_update_violation_q <= 1'b0;
    end else if (phase_w != DonePhase) begin
      ...
      if (phase_w == TrafficPhase) begin
        ...
        if (en_i) begin
          ...
          hold_count_q <= hold_count_q + 32'd1;
          if (enable_count_q == 32'd0) begin
      ...
      if (q_o != last_q_q) begin
        ...
        seen_q[5] <= 1'b1;
        if (!last_en_q) begin
          disabled_update_violation_q <= 1'b1;
          seen_q[10] <= 1'b1;
        end
      end
      if (!last_en_q && q_o == last_q_q) begin
        q_hold_match_count_q <= q_hold_match_count_q + 32'd1;
        seen_q[6] <= 1'b1;
      end
      if (last_en_q && q_o == last_d_q) begin
        update_applied_count_q <= update_applied_count_q + 32'd1;
        seen_q[7] <= 1'b1;
      end
  ...
  assign device_req_accepted_o = hold_count_q;
  assign device_rsp_accepted_o = q_change_count_q;
  assign host_rsp_accepted_o = data_toggle_count_q;
  assign rsp_queue_overflow_o = {31'd0, disabled_update_violation_q};
  ...
  assign real_toggle_subset_word2_o = {31'd0, en_i};
  assign real_toggle_subset_word3_o = {24'd0, data_mask_w};
  assign real_toggle_subset_word4_o = data_toggle_count_q;
  assign real_toggle_subset_word5_o = data_stable_count_q;
  assign real_toggle_subset_word6_o = enable_count_q;
  assign real_toggle_subset_word7_o = hold_count_q;
  assign real_toggle_subset_word8_o = q_change_count_q;
  assign real_toggle_subset_word9_o = q_hold_match_count_q;
  assign real_toggle_subset_word10_o = update_applied_count_q;
  assign real_toggle_subset_word11_o = reset_value_seen_count_q;
  ...
  assign focused_wave_word2_o = {31'd0, en_i};
  assign focused_wave_word3_o = {24'd0, data_mask_w};
  assign focused_wave_word4_o = enable_count_q ^ hold_count_q;
  ...
  assign oracle_expected_err_count_o = {31'd0, disabled_update_violation_q};
  ...
  assign oracle_semantic_case_acked_o = {16'd0, data_mask_w, {7'd0, en_i}};
...
endmodule : prim_xilinx_ultrascale_flop_en_gpu_cov_tb
