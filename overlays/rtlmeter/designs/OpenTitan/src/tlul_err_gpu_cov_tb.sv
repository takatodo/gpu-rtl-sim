...
//
// Minimal deterministic coverage harness for tlul_err.

module tlul_err_gpu_cov_tb (
  ...
  logic [31:0] invalid_opcode_count_q;
  logic [31:0] invalid_size_count_q;
  logic [31:0] instr_req_count_q;
  logic [31:0] aligned_req_count_q;
  logic [31:0] mask_case_count_q;

  logic req_fire_w;
  logic op_get_w;
  logic op_write_w;
  logic invalid_opcode_w;
  logic invalid_size_w;
  logic instr_req_w;
  logic aligned_req_w;
  ...
  function automatic tl_a_op_e choose_opcode(input logic [31:0] state);
    ...
    if (bucket >= (32'd128 - invalid_limit)) begin
      return tl_a_op_e'(3'h7);
    end
    return Get;
  endfunction
  ...
  function automatic logic [TL_AW-1:0] choose_address(input logic [31:0] state);
    ...
    if (pct_hit(state ^ 32'h4a11_a11e, cfg_rsp_error_pct_i)) begin
      return addr;
    end
  ...
  function automatic logic [TL_DBW-1:0] choose_mask(
    ...
    if (pct_hit(state ^ 32'hbad0_cafe, cfg_rsp_fill_target_i)) begin
      mask = TL_DBW'(state[TL_DBW-1:0]);
      return (mask == '0) ? TL_DBW'(4'b0010) : mask;
    end
  ...
  always_comb begin
    ...
    if (cfg_valid_i && phase_w == TrafficPhase &&
      ...
      tl_i.a_param = 3'd0;
      tl_i.a_size = choose_size(rand_state_q ^ cfg_rsp_family_i);
      tl_i.a_source = TL_AIW'((rand_state_q >> 8) & cfg_source_mask_i[TL_AIW-1:0]);
      tl_i.a_address = choose_address(rand_state_q ^ 32'h1234_5678);
      tl_i.a_mask = choose_mask(rand_state_q ^ 32'h3333_6666, tl_i.a_opcode, tl_i.a_size);
      tl_i.a_data = rand_state_q ^ cfg_req_data_mode_i ^ {cfg_req_data_hi_xor_i[15:0], 16'he221};
      tl_i.a_user = TL_A_USER_DEFAULT;
      tl_i.a_user.instr_type = choose_instr_type(rand_state_q ^ 32'hdead_beef);
    end
  end

  assign req_fire_w = tl_i.a_valid;
  assign op_get_w = req_fire_w && (tl_i.a_opcode == Get);
  assign op_write_w = req_fire_w && ((tl_i.a_opcode == PutFullData) ||
                                     (tl_i.a_opcode == PutPartialData));
  assign invalid_opcode_w = req_fire_w && !(op_get_w || op_write_w);
  assign invalid_size_w = req_fire_w && (tl_i.a_size > TL_SZW'(2));
  assign instr_req_w = req_fire_w &&
                       prim_mubi_pkg::mubi4_test_true_strict(tl_i.a_user.instr_type);
  assign aligned_req_w = req_fire_w && (tl_i.a_address[1:0] == 2'b00);
  assign req_digest_w = request_digest(tl_i);
  assign rsp_digest_w = req_digest_w ^ {31'd0, err_w};
  ...
  always_ff @(posedge clk_i) begin
    if (!cfg_valid_i) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_state_q <= cfg_seed_i ^ 32'he221_0001;
      ...
      invalid_opcode_count_q <= 32'd0;
      invalid_size_count_q <= 32'd0;
      instr_req_count_q <= 32'd0;
      aligned_req_count_q <= 32'd0;
      mask_case_count_q <= 32'd0;
    end else begin
      ...
      if (!rst_ni) begin
        ...
        if (req_fire_w) begin
          ...
          if (invalid_opcode_w) begin
            invalid_opcode_count_q <= invalid_opcode_count_q + 32'd1;
          end
          if (invalid_size_w) begin
            invalid_size_count_q <= invalid_size_count_q + 32'd1;
          end
          if (instr_req_w) begin
            instr_req_count_q <= instr_req_count_q + 32'd1;
          end
          if (aligned_req_w) begin
            aligned_req_count_q <= aligned_req_count_q + 32'd1;
          end
          if (tl_i.a_mask != TL_DBW'(4'b1111)) begin
            mask_case_count_q <= mask_case_count_q + 32'd1;
          end
          ...
          oracle_semantic_family_seen_o[3] <= oracle_semantic_family_seen_o[3] | invalid_opcode_w;
          oracle_semantic_family_seen_o[4] <= oracle_semantic_family_seen_o[4] | invalid_size_w;
          oracle_semantic_family_seen_o[5] <= oracle_semantic_family_seen_o[5] | instr_req_w;
          ...
          oracle_semantic_family_acked_o <= oracle_semantic_family_seen_o |
                                            {25'd0, err_w, instr_req_w, invalid_size_w,
                                             invalid_opcode_w, op_write_w, op_get_w, req_fire_w};
          oracle_semantic_case_seen_o <= oracle_semantic_case_seen_o |
              {25'd0, aligned_req_w, instr_req_w, invalid_size_w, invalid_opcode_w,
               op_write_w, op_get_w, req_fire_w};
          oracle_semantic_case_acked_o <= oracle_semantic_case_acked_o |
              {26'd0, tl_i.a_mask != TL_DBW'(4'b1111), err_w, instr_req_w,
               invalid_size_w, invalid_opcode_w, req_fire_w};
        end
      end
    end
  end
  ...
  assign toggle_bitmap_word1_o = host_req_accepted_o ^ oracle_expected_err_count_o ^
                                 oracle_observed_err_count_o ^ invalid_opcode_count_q;
  assign toggle_bitmap_word2_o = oracle_req_signature_o ^ oracle_stalled_req_signature_o ^
                                 progress_signature_o ^ invalid_size_count_q;
  ...
  assign real_toggle_subset_word16_o = last_req_digest_q ^ invalid_opcode_count_q ^
                                       invalid_size_count_q;
  assign real_toggle_subset_word17_o = last_rsp_digest_q ^ instr_req_count_q ^
                                       aligned_req_count_q ^ mask_case_count_q;
  ...
  assign focused_wave_word4_o = {24'd0, phase_w, tl_i.a_size, tl_i.a_opcode};
  assign focused_wave_word5_o = cfg_signature_o;
  assign focused_wave_word6_o = progress_signature_o;
  assign focused_wave_word7_o = {24'd0, err_w, instr_req_w, invalid_size_w,
                                 invalid_opcode_w, op_write_w, op_get_w, req_fire_w, rst_ni};

  logic unused_cfg;
  assign unused_cfg = ^{cfg_rsp_valid_pct_i, cfg_host_d_ready_pct_i,
                        cfg_device_a_ready_pct_i, cfg_req_burst_len_max_i,
