...
//
// Minimal deterministic coverage harness for tlul_err_resp.

module tlul_err_resp_gpu_cov_tb (
  ...
  logic [31:0] pending_rsp_cycles_q;
  logic [31:0] instr_error_rsp_count_q;
  logic [31:0] bus_error_rsp_count_q;
  ...
  always_comb begin
    tl_i = TL_H2D_DEFAULT;
    tl_i.d_ready = (phase_w == DrainPhase) ||
                   pct_hit(rand_state_q ^ 32'h99aa_7733, cfg_host_d_ready_pct_i);
    if (cfg_valid_i && phase_w == TrafficPhase && tl_o.a_ready &&
      ...
      tl_i.a_data = rand_state_q ^ cfg_req_data_mode_i ^ {cfg_req_data_hi_xor_i[15:0], 16'he771};
  ...
  assign op_write_w = a_ack_w && ((tl_i.a_opcode == PutFullData) ||
                                  (tl_i.a_opcode == PutPartialData));
  assign instr_req_w = prim_mubi_pkg::mubi4_test_true_strict(tl_i.a_user.instr_type);
  assign pending_rsp_w = tl_o.d_valid && !tl_i.d_ready;
  ...
  always_ff @(posedge clk_i) begin
    if (!cfg_valid_i) begin
      ...
      drain_cycle_q <= 32'd0;
      rand_state_q <= cfg_seed_i ^ 32'h51e1_d00d;
      ...
      pending_rsp_cycles_q <= 32'd0;
      instr_error_rsp_count_q <= 32'd0;
      bus_error_rsp_count_q <= 32'd0;
    end else begin
      ...
      if (!rst_ni) begin
        ...
        if (pending_rsp_w) begin
          pending_rsp_cycles_q <= pending_rsp_cycles_q + 32'd1;
        end
        if (a_ack_w) begin
          ...
          oracle_semantic_family_seen_o[3] <= oracle_semantic_family_seen_o[3] | instr_req_w;
          oracle_semantic_family_seen_o[4] <= 1'b1;
          oracle_semantic_case_seen_o <= oracle_semantic_case_seen_o |
              {26'd0, instr_req_w, op_get_w, op_write_w, tl_o.a_ready, tl_i.a_valid, a_ack_w};
        end
        if (tl_o.d_valid) begin
          device_rsp_accepted_o <= device_rsp_accepted_o + 32'd1;
          oracle_semantic_family_seen_o[5] <= 1'b1;
        end
        if (d_ack_w) begin
          ...
          if (tl_o.d_data == DataWhenInstrError) begin
            instr_error_rsp_count_q <= instr_error_rsp_count_q + 32'd1;
          end
          if (tl_o.d_data == DataWhenError) begin
            bus_error_rsp_count_q <= bus_error_rsp_count_q + 32'd1;
          end
          ...
          oracle_semantic_family_acked_o[3] <= oracle_semantic_family_acked_o[3] |
                                               (tl_o.d_data == DataWhenInstrError);
          oracle_semantic_family_acked_o[4] <= oracle_semantic_family_acked_o[4] |
                                               (tl_o.d_data == DataWhenError);
          oracle_semantic_case_acked_o <= oracle_semantic_case_acked_o |
              {26'd0, pending_rsp_w, tl_o.d_error, tl_o.d_opcode == AccessAckData,
               tl_i.d_ready, tl_o.d_valid, d_ack_w};
        end
      end
    end
  end
  ...
  assign toggle_bitmap_word2_o = oracle_req_signature_o ^ oracle_stalled_req_signature_o ^
                                 progress_signature_o ^ pending_rsp_cycles_q;
  ...
  assign real_toggle_subset_word16_o = last_req_digest_q ^ pending_rsp_cycles_q;
  assign real_toggle_subset_word17_o = last_rsp_digest_q ^ instr_error_rsp_count_q ^
                                       bus_error_rsp_count_q;
  ...
  assign focused_wave_word4_o = {23'd0, phase_w, tl_o.d_opcode, tl_i.a_opcode};
  assign focused_wave_word5_o = cfg_signature_o;
  assign focused_wave_word6_o = progress_signature_o;
  assign focused_wave_word7_o = {25'd0, tl_o.d_error, tl_o.d_valid, tl_o.a_ready,
                                 tl_i.a_valid, tl_i.d_ready, a_ack_w, d_ack_w};

  logic unused_cfg;
  assign unused_cfg = ^{cfg_rsp_valid_pct_i, cfg_device_a_ready_pct_i, cfg_req_fill_target_i,
                        cfg_req_burst_len_max_i, cfg_access_ack_data_pct_i,
                        cfg_rsp_error_pct_i, cfg_rsp_fill_target_i, cfg_rsp_delay_max_i,
                        cfg_rsp_family_i, cfg_rsp_delay_mode_i, cfg_rsp_data_mode_i,
