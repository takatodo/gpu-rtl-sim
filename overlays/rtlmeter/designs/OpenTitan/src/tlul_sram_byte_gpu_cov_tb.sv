...
//
// Minimal deterministic coverage harness for tlul_sram_byte.

module tlul_sram_byte_gpu_cov_tb (
  ...
  tl_d_op_e sram_rsp_opcode_q;
  logic [TL_SZW-1:0] sram_rsp_size_q;
  logic [TL_AIW-1:0] sram_rsp_source_q;
  ...
  logic [31:0] error_i_count_q;
  logic [31:0] error_o_count_q;
  ...
  logic [TL_DW-1:0] req_raw_data_w;
  logic [DataFullWidth-1:0] req_data_intg_full_w;
  logic [DataFullWidth-1:0] rsp_data_intg_full_w;

  initial clk_i = 1'b0;
  always #5 clk_i = ~clk_i;

  tlul_data_integ_enc u_req_data_intg (
    .data_i      (req_raw_data_w),
    .data_intg_o (req_data_intg_full_w)
  );

  tlul_data_integ_enc u_rsp_data_intg (
    .data_i      (sram_rsp_data_q),
    .data_intg_o (rsp_data_intg_full_w)
  );
  ...
  function automatic logic [TL_AW-1:0] choose_address(input logic [31:0] state);
    ...
    if ((cfg_req_address_mode_i[0]) && pct_hit(state ^ 32'hb17e_a551, 32'd20)) begin
      return addr;
    end
  ...
  function automatic logic [TL_DW-1:0] expand_mask(input logic [TL_DBW-1:0] mask);
    logic [TL_DW-1:0] expanded;
    for (int unsigned byte_idx = 0; byte_idx < TL_DBW; byte_idx++) begin
      expanded[byte_idx*8 +: 8] = {8{mask[byte_idx]}};
    end
    return expanded;
  endfunction
  ...
  function automatic logic [31:0] response_digest(input tl_d2h_t tl);
    ...
    digest ^= tl.d_data;
    digest ^= 32'(tl.d_source) ^ 32'(tl.d_sink) ^ 32'(tl.d_user.data_intg);
    return digest;
  endfunction
  ...
  assign req_raw_data_w = (rand_state_q ^ {cfg_req_data_hi_xor_i[15:0], cfg_req_data_mode_i[15:0]}) +
                          cfg_seed_i;
  ...
  always_comb begin
    ...
    if (phase_w == TrafficPhase) begin
      ...
      tl_i.a_data = req_raw_data_w;
      tl_i.a_user = TL_A_USER_DEFAULT;
      tl_i.a_user.data_intg = req_data_intg_full_w[DataFullWidth-1:DataMaxWidth];
    end
    ...
    error_i_w = (phase_w == TrafficPhase) &&
                pct_hit(rand_state_q ^ 32'he110_b17e, cfg_rsp_error_pct_i >> 2);
    ...
    tl_sram_i = TL_D2H_DEFAULT;
    tl_sram_i.a_ready = sram_gnt_w;
    tl_sram_i.d_valid = sram_rvalid_w;
    tl_sram_i.d_opcode = sram_rsp_opcode_q;
    tl_sram_i.d_param = '0;
    tl_sram_i.d_size = sram_rsp_size_q;
    tl_sram_i.d_source = sram_rsp_source_q;
    tl_sram_i.d_sink = '0;
    tl_sram_i.d_data = sram_rsp_data_q;
    tl_sram_i.d_user = TL_D_USER_DEFAULT;
    tl_sram_i.d_user.data_intg = rsp_data_intg_full_w[DataFullWidth-1:DataMaxWidth];
    tl_sram_i.d_error = |sram_rsp_error_q;
  end
  ...
  assign sram_req_w = tl_sram_o.a_valid;
  assign sram_we_w = tl_sram_o.a_opcode != Get;
  assign sram_addr_w = tl_sram_o.a_address[7:0];
  assign sram_wdata_w = tl_sram_o.a_data;
  assign sram_wmask_w = expand_mask(tl_sram_o.a_mask);
  assign sram_accept_w = tl_sram_o.a_valid && sram_gnt_w;
  assign sram_read_accept_w = sram_accept_w && (tl_sram_o.a_opcode == Get);
  assign sram_write_accept_w = sram_accept_w && (tl_sram_o.a_opcode != Get);
  assign req_digest_w = request_digest(tl_i);
  assign rsp_digest_w = response_digest(tl_o);
  assign sram_digest_w = sram_digest(sram_req_w, sram_we_w, sram_addr_w, sram_wdata_w,
                                     sram_wmask_w, sram_rvalid_w, sram_rdata_w, sram_rerror_w);
  ...
  always_ff @(posedge clk_i) begin
    if (!cfg_valid_i) begin
      ...
      traffic_cycle_q <= '0;
      drain_cycle_q <= '0;
      rand_state_q <= cfg_seed_i ^ 32'hb17e_5eed;
      first_handshake_seen_q <= 1'b0;
      sram_rsp_pending_q <= 1'b0;
      sram_rsp_delay_q <= '0;
      sram_rsp_opcode_q <= AccessAck;
      sram_rsp_size_q <= '0;
      sram_rsp_source_q <= '0;
      sram_rsp_data_q <= '0;
      sram_rsp_error_q <= '0;
      ...
      if (!rst_ni) begin
        ...
        if (sram_rsp_pending_q) begin
          ...
          end else if (sram_rvalid_w && tl_sram_o.d_ready) begin
            sram_rsp_pending_q <= 1'b0;
          end
        end
        if (sram_accept_w) begin
          ...
          sram_rsp_opcode_q <= (tl_sram_o.a_opcode == Get) ? AccessAckData : AccessAck;
          sram_rsp_size_q <= tl_sram_o.a_size;
          sram_rsp_source_q <= tl_sram_o.a_source;
          sram_rsp_data_q <= prng_next(rand_state_q ^ cfg_rsp_data_mode_i) ^
                             cfg_rsp_data_hi_xor_i ^ {24'd0, sram_addr_w} ^ tl_sram_o.a_data;
        ...
        if (req_fire_w) begin
          ...
          oracle_semantic_family_seen_o <= oracle_semantic_family_seen_o |
                                           {26'd0, error_i_w, compound_txn_w, is_partial_write_w,
                                            is_full_write_w, is_read_w, req_fire_w};
        ...
        if (sram_rvalid_w && tl_sram_o.d_ready) begin
        ...
        if (error_i_w) begin
          error_i_count_q <= error_i_count_q + 32'd1;
        end
        if (error_o_w) begin
          error_o_count_q <= error_o_count_q + 32'd1;
        end
        ...
        if (rsp_fire_w) begin
          ...
          oracle_semantic_family_acked_o <= oracle_semantic_family_acked_o |
                                            {27'd0, error_o_w, tl_o.d_error,
                                             tl_o.d_opcode == AccessAckData, tl_o.d_valid,
                                             rsp_fire_w};
          ...
          if (tl_o.d_error || error_o_w) begin
  ...
  assign real_toggle_subset_word6_o = sram_req_count_q;
  assign real_toggle_subset_word7_o = sram_read_count_q;
  assign real_toggle_subset_word8_o = sram_write_count_q;
  assign real_toggle_subset_word9_o = sram_rsp_count_q;
  assign real_toggle_subset_word10_o = host_stall_count_q;
  assign real_toggle_subset_word11_o = error_i_count_q;
  assign real_toggle_subset_word12_o = error_o_count_q;
  assign real_toggle_subset_word13_o = error_rsp_count_q;
  assign real_toggle_subset_word14_o = compound_count_q;
  assign real_toggle_subset_word15_o = addr_signature_q ^ data_signature_q;
  assign real_toggle_subset_word16_o = sram_signature_q ^ mask_signature_q;
  assign real_toggle_subset_word17_o = wr_collision_count_q ^ write_pending_count_q ^
                                       oracle_req_signature_delta_o;

  assign focused_wave_word0_o = global_cycle_q;
  assign focused_wave_word1_o = traffic_cycle_q;
  assign focused_wave_word2_o = {16'd0, alert_w, error_o_w, error_i_w, compound_txn_w,
                                 sram_req_w, sram_gnt_w, sram_we_w, sram_rvalid_w,
                                 sram_addr_w};
