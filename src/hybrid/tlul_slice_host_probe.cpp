...
struct ProbeSummary {
  ...
  bool raw_root_eval_present = false;
  uint32_t raw_root_eval_steps = 0;
  EdgeSummary raw_root_eval;
};
...
ProbeConfig parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--raw-root-eval-state-out") {
      cfg.raw_root_eval_state_out = (i + 1) < argc ? argv[++i] : "";
  ...
  if (cfg.raw_root_eval_steps == 0U && !cfg.raw_root_eval_state_out.empty()) {
    fail("--raw-root-eval-state-out requires --raw-root-eval-steps");
  }
  return cfg;
}
...
EdgeSummary run_raw_root_eval(
  ...
  (void)steps;
  (void)state_out;
  fail("--raw-root-eval-steps requires ROOT_EVAL_FN support in the compiled probe");
#else
...
void emit_raw_root_eval(const ProbeSummary& summary) {
  std::cout << "  \"raw_root_eval\": ";
  if (!summary.raw_root_eval_present) {
    std::cout << "null,\n";
    return;
  }
  const auto& run = summary.raw_root_eval;
  std::cout << "{\n";
  std::cout << "    \"steps\": " << summary.raw_root_eval_steps << ",\n";
  std::cout << "    \"state_out\": \"" << run.dump_state << "\",\n";
  std::cout << "    \"result\": {\n";
  std::cout << "      \"sim_time\": " << run.sim_time << ",\n";
  std::cout << "      \"dump_state\": \"" << run.dump_state << "\",\n";
  std::cout << "      \"done_o\": " << run.done_o << ",\n";
  std::cout << "      \"progress_cycle_count_o\": " << run.progress_cycle_count_o << ",\n";
  std::cout << "      \"progress_signature_o\": " << run.progress_signature_o << ",\n";
  std::cout << "      \"toggle_bitmap_word0_o\": " << run.toggle_bitmap_word0_o << ",\n";
...
int main(int argc, char** argv) {
  try {
    ...
    if (cfg.raw_root_eval_steps != 0U) {
      ...
      summary.raw_root_eval_steps = cfg.raw_root_eval_steps;
      summary.raw_root_eval = run_raw_root_eval(
          model,
          context,
          root,
          cfg.raw_root_eval_steps,
          cfg.raw_root_eval_state_out);
    }
