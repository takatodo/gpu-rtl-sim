#!/usr/bin/env python3
"""Materialize and time the scientific CIRCT batched reduction candidate."""
...
OUT_DIR = Path("artifacts/scientific_circt/batched_reduction")
REPORT = Path("reports/scientific_circt_batched_reduction.json")

FIRRTL = """FIRRTL version 4.0.0
circuit BatchedReduction :
  public module BatchedReduction :
    input x0 : UInt<16>
    input x1 : UInt<16>
    input x2 : UInt<16>
    input x3 : UInt<16>
    ...
    node s45 = add(x4, x5)
    node s67 = add(x6, x7)
    node s03 = add(s01, s23)
    node s47 = add(s45, s67)
    connect y, add(s03, s47)
"""
...
static uint32_t ref(const std::array<uint16_t, 8>& x) {
  uint32_t y = 0;
  for (uint16_t v : x) y += v;
  return y;
}
...
int main(int argc, char **argv) {
  ...
  for (const auto &c : cases) {
    top.x0=c[0]; top.x1=c[1]; top.x2=c[2]; top.x3=c[3]; top.x4=c[4]; top.x5=c[5]; top.x6=c[6]; top.x7=c[7];
    top.eval();
    if (top.y != ref(c)) {
      std::cerr << "batched_reduction mismatch got=" << top.y << " exp=" << ref(c) << "\n";
      return 1;
    }
  }
  top.final();
  std::cout << "TEST PASSED batched_reduction_cpu_reference cases=" << cases.size() << "\n";
  return 0;
}
'''
...
static void fill(std::vector<In>& in) {
  for (size_t i = 0; i < in.size(); ++i) for (int j = 0; j < 8; ++j) in[i].x[j] = static_cast<uint16_t>((i * 17u + j * 13u + 1u) & 0xffffu);
}
__host__ __device__ static Out compute(const In& in, int inner) {
  uint64_t base = 0, y = 0;
  for (int j = 0; j < 8; ++j) base += in.x[j];
  for (int r = 0; r < inner; ++r) y = (y + base) ^ static_cast<uint64_t>(r & 15);
  return Out{y};
}
...
def materialize_and_measure(shape: str, repeat: int, inner_repeat: int, out_dir: Path = OUT_DIR) -> dict[str, Any]:
    ...
    if steps != 1:
        raise ValueError("batched_reduction currently supports Nx1 shapes")
    out_dir.mkdir(parents=True, exist_ok=True)
    fir, sv, harness, obj = out_dir / "batched_reduction.fir", out_dir / "batched_reduction.sv", out_dir / "tb_batched_reduction.cpp", out_dir / "obj_dir"
    cu, bin_path = out_dir / "batched_reduction_gpu_timing.cu", out_dir / "batched_reduction_gpu_timing"
    fir.write_text(FIRRTL, encoding="utf-8"); harness.write_text(HARNESS, encoding="utf-8"); cu.write_text(CUDA, encoding="utf-8")
    report: dict[str, Any] = {"schema_version": 1, "surface": "scientific_circt_testbench", "candidate": "batched_reduction", "shape": shape, "inner_repeat": inner_repeat, "commands": [], "toolchain": {"firtool": bool(shutil.which("firtool")), "verilator": bool(shutil.which("verilator")), "nvcc": bool(shutil.which("nvcc"))}, "non_claims": ["not_a_general_rtl_speedup_claim", "not_automatic_partitioning"]}
    for name, cmd in [
        ("firtool", ["firtool", fir.as_posix(), "-o", sv.as_posix()]),
        ("verilator_build", ["verilator", "--cc", "--exe", "--timing", "-Wno-fatal", "--prefix", "Vsim", "--top-module", "BatchedReduction", "-Mdir", obj.as_posix(), sv.as_posix(), harness.as_posix(), "--build"]),
