#!/usr/bin/env python3
"""Materialize and time a microGPT-derived MLP CIRCT slice."""
...
OUT_DIR = Path("artifacts/scientific_circt/microgpt_mlp_slice")
REPORT = Path("reports/scientific_circt_microgpt_mlp_slice.json")

FIRRTL = """FIRRTL version 4.0.0
circuit MicrogptMlpSlice :
  public module MicrogptMlpSlice :
    input x0 : UInt<8>
    input x1 : UInt<8>
    input x2 : UInt<8>
    input x3 : UInt<8>
    ...
    node h0sum_a = add(mul(x0, UInt<4>(3)), mul(x1, UInt<4>(5)))
    node h0sum_b = add(mul(x2, UInt<4>(7)), mul(x3, UInt<4>(11)))
    node h0sum = add(h0sum_a, h0sum_b)
    node h0 = mux(gt(h0sum, UInt<16>(900)), sub(h0sum, UInt<16>(900)), UInt<16>(0))

    node h1sum_a = add(mul(x0, UInt<4>(13)), mul(x1, UInt<4>(2)))
    node h1sum_b = add(mul(x2, UInt<4>(5)), mul(x3, UInt<4>(3)))
    node h1sum = add(h1sum_a, h1sum_b)
    node h1 = mux(gt(h1sum, UInt<16>(1100)), sub(h1sum, UInt<16>(1100)), UInt<16>(0))

    node h2sum_a = add(mul(x0, UInt<4>(2)), mul(x1, UInt<5>(17)))
    node h2sum_b = add(mul(x2, UInt<4>(3)), mul(x3, UInt<4>(5)))
    node h2sum = add(h2sum_a, h2sum_b)
    node h2 = mux(gt(h2sum, UInt<16>(1300)), sub(h2sum, UInt<16>(1300)), UInt<16>(0))

    node h3sum_a = add(mul(x0, UInt<4>(11)), mul(x1, UInt<4>(7)))
    node h3sum_b = add(mul(x2, UInt<4>(13)), mul(x3, UInt<4>(2)))
    node h3sum = add(h3sum_a, h3sum_b)
    node h3 = mux(gt(h3sum, UInt<16>(1500)), sub(h3sum, UInt<16>(1500)), UInt<16>(0))

    node y0a = add(mul(h0, UInt<5>(19)), mul(h1, UInt<5>(23)))
    node y0b = add(mul(h2, UInt<5>(29)), mul(h3, UInt<5>(31)))
    node y1a = add(mul(h0, UInt<5>(7)), mul(h1, UInt<5>(17)))
    node y1b = add(mul(h2, UInt<5>(11)), mul(h3, UInt<5>(13)))
    connect y0, add(y0a, y0b)
    connect y1, add(y1a, y1b)
"""
...
static Out ref(const std::array<uint8_t, 4>& x) {
  ...
  return Out{h0*19 + h1*23 + h2*29 + h3*31, h0*7 + h1*17 + h2*11 + h3*13};
}
int main(int argc, char **argv) {
  Verilated::commandArgs(argc, argv);
  Vsim top;
  const std::array<std::array<uint8_t, 4>, 4> cases = {{{1,2,3,4}, {63,64,65,66}, {127,111,97,83}, {255,241,227,211}}};
  for (const auto &c : cases) {
    top.x0=c[0]; top.x1=c[1]; top.x2=c[2]; top.x3=c[3]; top.eval();
    Out e = ref(c);
    if (top.y0 != e.y0 || top.y1 != e.y1) { std::cerr << "microgpt_mlp_slice mismatch\n"; return 1; }
  }
  top.final();
  std::cout << "TEST PASSED microgpt_mlp_slice_cpu_reference cases=" << cases.size() << "\n";
  return 0;
}
'''
...
static void fill(std::vector<In>& in) {
  for (size_t i = 0; i < in.size(); ++i) for (int j = 0; j < 4; ++j) in[i].x[j] = uint8_t((i*31u+j*17u+3u)&0xffu);
}
...
__host__ __device__ static Out eval_once(const In& in) {
  uint64_t h0 = relu_sub(uint64_t(in.x[0])*3 + uint64_t(in.x[1])*5 + uint64_t(in.x[2])*7 + uint64_t(in.x[3])*11, 900);
  uint64_t h1 = relu_sub(uint64_t(in.x[0])*13 + uint64_t(in.x[1])*2 + uint64_t(in.x[2])*5 + uint64_t(in.x[3])*3, 1100);
  uint64_t h2 = relu_sub(uint64_t(in.x[0])*2 + uint64_t(in.x[1])*17 + uint64_t(in.x[2])*3 + uint64_t(in.x[3])*5, 1300);
  uint64_t h3 = relu_sub(uint64_t(in.x[0])*11 + uint64_t(in.x[1])*7 + uint64_t(in.x[2])*13 + uint64_t(in.x[3])*2, 1500);
  return Out{h0*19 + h1*23 + h2*29 + h3*31, h0*7 + h1*17 + h2*11 + h3*13};
}
__host__ __device__ static Out compute(const In& in, int inner) {
  Out base = eval_once(in), out{0, 0};
  for (int r = 0; r < inner; ++r) { out.y0 = (out.y0 + base.y0) ^ uint64_t((r * 7) & 63); out.y1 = (out.y1 + base.y1) ^ uint64_t((r * 11) & 127); }
  return out;
}
...
def materialize_and_measure(shape: str, repeat: int, inner_repeat: int, out_dir: Path = OUT_DIR) -> dict[str, Any]:
    ...
    if steps != 1:
        raise ValueError("microgpt_mlp_slice currently supports Nx1 shapes")
    out_dir.mkdir(parents=True, exist_ok=True)
    fir, sv, harness, obj = out_dir / "microgpt_mlp_slice.fir", out_dir / "microgpt_mlp_slice.sv", out_dir / "tb_microgpt_mlp_slice.cpp", out_dir / "obj_dir"
    cu, bin_path = out_dir / "microgpt_mlp_slice_gpu_timing.cu", out_dir / "microgpt_mlp_slice_gpu_timing"
    fir.write_text(FIRRTL, encoding="utf-8"); harness.write_text(HARNESS, encoding="utf-8"); cu.write_text(CUDA, encoding="utf-8")
    report: dict[str, Any] = {"schema_version": 1, "surface": "scientific_circt_testbench", "candidate": "microgpt_mlp_slice", "shape": shape, "inner_repeat": inner_repeat, "commands": [], "toolchain": {"firtool": bool(shutil.which("firtool")), "verilator": bool(shutil.which("verilator")), "nvcc": bool(shutil.which("nvcc"))}, "non_claims": ["not_full_microgpt_execution", "not_full_mlp_width", "not_a_general_rtl_speedup_claim", "not_automatic_partitioning"]}
    for name, cmd in [
        ("firtool", ["firtool", fir.as_posix(), "-o", sv.as_posix()]),
        ("verilator_build", ["verilator", "--cc", "--exe", "--timing", "-Wno-fatal", "--prefix", "Vsim", "--top-module", "MicrogptMlpSlice", "-Mdir", obj.as_posix(), sv.as_posix(), harness.as_posix(), "--build"]),
