#!/usr/bin/env python3
"""Materialize and time the scientific CIRCT 2D stencil tile candidate."""
...
OUT_DIR = Path("artifacts/scientific_circt/stencil_2d_tile")
REPORT = Path("reports/scientific_circt_stencil_2d_tile.json")

FIRRTL = """FIRRTL version 4.0.0
circuit Stencil2dTile :
  public module Stencil2dTile :
    input n : UInt<16>
    input s : UInt<16>
    input e : UInt<16>
    input w : UInt<16>
    ...
    node ns = add(n, s)
    node ew = add(e, w)
    node neighbors = add(ns, ew)
    node c4 = shl(c, 2)
    connect y, add(c4, neighbors)
"""
...
#include <cstdint>
#include <iostream>
struct Case { uint16_t n, s, e, w, c; };
static uint32_t ref(const Case& x) { return static_cast<uint32_t>(x.c) * 4u + x.n + x.s + x.e + x.w; }
int main(int argc, char **argv) {
  Verilated::commandArgs(argc, argv);
  Vsim top;
  const std::array<Case, 4> cases = {{{1,2,3,4,5},{9,10,11,12,13},{255,17,19,23,29},{65535,3,4,5,6}}};
  for (const auto &x : cases) {
    top.n=x.n; top.s=x.s; top.e=x.e; top.w=x.w; top.c=x.c; top.eval();
    if (top.y != ref(x)) { std::cerr << "stencil_2d_tile mismatch got=" << top.y << " exp=" << ref(x) << "\n"; return 1; }
  }
  top.final();
  std::cout << "TEST PASSED stencil_2d_tile_cpu_reference cases=" << cases.size() << "\n";
  return 0;
}
'''
...
#include <iostream>
#include <vector>
struct In { uint16_t n, s, e, w, c; };
struct Out { uint64_t y; };
static void fill(std::vector<In>& in) {
  for (size_t i = 0; i < in.size(); ++i)
    in[i] = {static_cast<uint16_t>((i*17u+1u)&0xffffu), static_cast<uint16_t>((i*19u+2u)&0xffffu),
             static_cast<uint16_t>((i*23u+3u)&0xffffu), static_cast<uint16_t>((i*29u+4u)&0xffffu),
             static_cast<uint16_t>((i*31u+5u)&0xffffu)};
}
__host__ __device__ static Out compute(const In& x, int inner) {
  uint64_t base = static_cast<uint64_t>(x.c) * 4u + x.n + x.s + x.e + x.w, y = 0;
  for (int r = 0; r < inner; ++r) y = (y + base) ^ static_cast<uint64_t>(r & 7);
  return Out{y};
}
...
def materialize_and_measure(shape: str, repeat: int, inner_repeat: int, out_dir: Path = OUT_DIR) -> dict[str, Any]:
    ...
    if steps != 1:
        raise ValueError("stencil_2d_tile currently supports Nx1 shapes")
    out_dir.mkdir(parents=True, exist_ok=True)
    fir, sv, harness, obj = out_dir / "stencil_2d_tile.fir", out_dir / "stencil_2d_tile.sv", out_dir / "tb_stencil_2d_tile.cpp", out_dir / "obj_dir"
    cu, bin_path = out_dir / "stencil_2d_tile_gpu_timing.cu", out_dir / "stencil_2d_tile_gpu_timing"
    fir.write_text(FIRRTL, encoding="utf-8"); harness.write_text(HARNESS, encoding="utf-8"); cu.write_text(CUDA, encoding="utf-8")
    report: dict[str, Any] = {"schema_version": 1, "surface": "scientific_circt_testbench", "candidate": "stencil_2d_tile", "shape": shape, "inner_repeat": inner_repeat, "commands": [], "toolchain": {"firtool": bool(shutil.which("firtool")), "verilator": bool(shutil.which("verilator")), "nvcc": bool(shutil.which("nvcc"))}, "non_claims": ["not_a_general_rtl_speedup_claim", "not_automatic_partitioning"]}
    for name, cmd in [
        ("firtool", ["firtool", fir.as_posix(), "-o", sv.as_posix()]),
        ("verilator_build", ["verilator", "--cc", "--exe", "--timing", "-Wno-fatal", "--prefix", "Vsim", "--top-module", "Stencil2dTile", "-Mdir", obj.as_posix(), sv.as_posix(), harness.as_posix(), "--build"]),
