...
NEXT_MEASUREMENT_AFTER_PAGED_ATTENTION_KV_REPEAT_MEDIAN_REFRESH_GATE = (
    REPO_ROOT
    / "config"
    / "scaling_gates"
...
class FullItaMhaAndLargerPagedKvNextGateTest(unittest.TestCase):
    ...
    def test_next_measurement_after_paged_attention_kv_cache_repeat_median_refresh_selects_config_generation_breadth(self) -> None:
        gate = json.loads(
            NEXT_MEASUREMENT_AFTER_PAGED_ATTENTION_KV_REPEAT_MEDIAN_REFRESH_GATE.read_text(encoding="utf-8")
        )
        selection = json.loads(SELECTION.read_text(encoding="utf-8"))
        combined = "\n".join(
            [
        ...
        self.assertEqual(gate["status"], "selected_config_generation_validation_breadth_next")
        self.assertEqual(
            gate["source_completion_gate"],
            "config/scaling_gates/public_benchmark_pack_externalization_completion_after_paged_attention_kv_cache_repeat_median_gate.json",
        )
        self.assertEqual(gate["current_priority"], "define_config_generation_validation_breadth_gate")
        self.assertEqual(gate["decision"]["selected_next_workstream"], "config_generation_validation_breadth")
        self.assertEqual(gate["decision"]["selected_first_gate"], "define_config_generation_validation_breadth_gate")
        reviewed = {option["name"]: option["decision"] for option in gate["candidate_review"]}
        self.assertEqual(reviewed["config_generation_validation_breadth"], "selected")
        self.assertEqual(reviewed["paged_attention_kv_cache_scale_up_continuation"], "deferred")
        self.assertEqual(reviewed["resident_execution_optimization"], "deferred")
        self.assertEqual(reviewed["quantized_kv_cache_boundary"], "deferred")
        self.assertEqual(reviewed["full_ita_mha_additional_shapes"], "deferred")
        ...
        self.assertFalse(policy["raw_full_state_equality_required"])
        self.assertFalse(policy["reports_and_artifacts_are_source_of_truth"])
        self.assertEqual(gate["next_task"], "define_config_generation_validation_breadth_gate")
        self.assertIn("not a new measurement result", gate["non_claims"])
        self.assertIn("reports and artifacts are generated evidence, not source of truth", gate["non_claims"])
    ...
    def test_config_generation_validation_breadth_gate_defines_tracked_template_set(self) -> None:
        gate = json.loads(CONFIG_GENERATION_VALIDATION_BREADTH_GATE.read_text(encoding="utf-8"))
        selection = json.loads(SELECTION.read_text(encoding="utf-8"))
        combined = "\n".join(
            [
                README.read_text(encoding="utf-8"),
        ...
        self.assertEqual(gate["gate"], "config_generation_validation_breadth_gate")
        self.assertEqual(gate["status"], "defined_config_generation_validation_breadth")
        self.assertEqual(
            gate["source_selection_gate"],
            "config/scaling_gates/next_measurement_selection_after_paged_attention_kv_cache_repeat_median_refresh_gate.json",
        )
        self.assertEqual(gate["current_priority"], "run_config_generation_validation_breadth_dry_run_gate")

        templates = gate["selected_scope"]["templates"]
        ...
        for template in templates:
            template_path = REPO_ROOT / template["template"]
            self.assertTrue(template_path.exists())
            self.assertEqual(
                subprocess.run(
                    ["git", "ls-files", "--error-unmatch", template["template"]],
                    cwd=REPO_ROOT,
                    text=True,
                    capture_output=True,
                ).returncode,
            ...
            self.assertEqual(payload["build"]["host_probe_builder"], "src/tools/build_host_probe.py")
            self.assertIn("clock_field", payload["build"]["host_probe"])
            self.assertIn("reset_field", payload["build"]["host_probe"])

        self.assertIn("build.host_probe.clock_field", gate["metadata_fields_under_review"])
        self.assertIn("build.host_probe.reset_field", gate["metadata_fields_under_review"])
        self.assertIn("all selected templates are tracked by git", gate["required_static_checks"])
        self.assertEqual(len(gate["dry_run_commands"]), 6)
        self.assertTrue(all(command.endswith("--dry-run") for command in gate["dry_run_commands"]))
    ...
    def test_config_generation_validation_breadth_dry_run_gate_records_command_plan_pass(self) -> None:
        gate = json.loads(CONFIG_GENERATION_VALIDATION_BREADTH_DRY_RUN_GATE.read_text(encoding="utf-8"))
        selection = json.loads(SELECTION.read_text(encoding="utf-8"))
        combined = "\n".join(
            [
                README.read_text(encoding="utf-8"),
        ...
        self.assertEqual(gate["gate"], "config_generation_validation_breadth_dry_run_gate")
        self.assertEqual(gate["status"], "passed_config_generation_validation_breadth_dry_run")
        self.assertEqual(gate["source_definition_gate"], "config/scaling_gates/config_generation_validation_breadth_gate.json")
        self.assertEqual(gate["current_priority"], "review_config_generation_validation_breadth_dry_run_gate")
        result = gate["dry_run_result"]
        self.assertTrue(result["all_commands_exited_zero"])
        self.assertEqual(result["command_count"], 6)
        self.assertTrue(result["all_emit_generic_host_probe_builder"])
        self.assertFalse(result["makefile_host_probe_target_observed"])
        self.assertTrue(result["all_emit_coverage_output_compare_policy"])
        self.assertTrue(result["all_outputs_are_command_plans_only"])

        self.assertEqual(len(gate["commands"]), 6)
        for command in gate["commands"]:
            self.assertEqual(command["exit_code"], 0)
            self.assertIn(" --dry-run", command["command"])
            self.assertIn("src/tools/build_host_probe.py", command["observed_host_probe_builder"])
            self.assertEqual(command["observed_compare_policy"], "coverage_output_equivalence")
            self.assertTrue(command["json_out"].startswith("reports/"))
    ...
    def test_config_generation_validation_breadth_dry_run_review_selects_execution_gate_definition(self) -> None:
        gate = json.loads(CONFIG_GENERATION_VALIDATION_BREADTH_DRY_RUN_REVIEW_GATE.read_text(encoding="utf-8"))
        selection = json.loads(SELECTION.read_text(encoding="utf-8"))
        combined = "\n".join(
            [
                README.read_text(encoding="utf-8"),
        ...
        self.assertEqual(gate["gate"], "config_generation_validation_breadth_dry_run_review_gate")
        self.assertEqual(
            gate["status"],
            "reviewed_config_generation_validation_breadth_dry_run_accepts_command_plan_boundary",
        )
        self.assertEqual(
        ...
        self.assertEqual(gate["current_priority"], "define_config_generation_validation_breadth_execution_gate")
        self.assertTrue(gate["review_decision"]["dry_run_boundary_accepted"])
        self.assertEqual(
            gate["review_decision"]["selected_next_task"],
            "define_config_generation_validation_breadth_execution_gate",
        )
        reviewed = gate["reviewed_result"]
        self.assertEqual(reviewed["command_count"], 6)
        self.assertTrue(reviewed["all_commands_exited_zero"])
        self.assertTrue(reviewed["all_emit_generic_host_probe_builder"])
        self.assertFalse(reviewed["makefile_host_probe_target_observed"])
        self.assertTrue(reviewed["all_outputs_are_command_plans_only"])
        self.assertIn("dry-run does not prove Verilator build success", gate["weak_points"])
        self.assertEqual(gate["required_next_gate"]["name"], "define_config_generation_validation_breadth_execution_gate")
        ...
        self.assertFalse(policy["runtime_or_abi_change_allowed_by_this_gate"])
        self.assertFalse(policy["new_workload_allowed_by_this_gate"])
        self.assertFalse(policy["fresh_build_run_compare_claim_allowed"])
        self.assertFalse(policy["reports_and_artifacts_are_source_of_truth"])
