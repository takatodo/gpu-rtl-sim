...
import scientific_circt_microgpt_attention_head as attention  # noqa: E402


class ScientificCirctMicrogptAttentionHeadTest(HybridCliTestCase):
    def test_shape_validation(self) -> None:
        self.assertEqual(attention._shape("1024x1"), (1024, 1))
        with self.assertRaises(ValueError):
            attention._shape("1024")
    ...
    def test_materialize_and_measure_summarizes_mocked_toolchain(self) -> None:
        ...
        def fake_run(argv: list[str]) -> dict[str, object]:
            ...
            if argv[0].endswith("microgpt_attention_head_gpu_timing"):
                stdout = json.dumps(sample)
            return {"argv": [attention._sanitize(arg) for arg in argv], "returncode": 0, "stdout": stdout, "stderr": ""}

        with tempfile.TemporaryDirectory() as temp_dir, \
             mock.patch.object(attention.shutil, "which", return_value="/usr/bin/tool"), \
             mock.patch.object(attention, "_run", side_effect=fake_run):
            report = attention.materialize_and_measure("1024x1", 5, 1000, Path(temp_dir))
