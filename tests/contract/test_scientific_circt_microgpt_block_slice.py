...
import scientific_circt_microgpt_block_slice as block  # noqa: E402


class ScientificCirctMicrogptBlockSliceTest(HybridCliTestCase):
    def test_shape_validation(self) -> None:
        self.assertEqual(block._shape("1024x1"), (1024, 1))
    ...
    def test_materialize_and_measure_summarizes_mocked_toolchain(self) -> None:
        ...
        def fake_run(argv: list[str]) -> dict[str, object]:
            ...
            if argv[0].endswith("microgpt_block_slice_gpu_timing"):
                stdout = json.dumps(sample)
            return {"argv": [block._sanitize(arg) for arg in argv], "returncode": 0, "stdout": stdout, "stderr": ""}

        with tempfile.TemporaryDirectory() as temp_dir, \
             mock.patch.object(block.shutil, "which", return_value="/usr/bin/tool"), \
             mock.patch.object(block, "_run", side_effect=fake_run):
            report = block.materialize_and_measure("1024x1", 5, 1000, Path(temp_dir))

        self.assertEqual(report["status"], "measured")
        self.assertEqual(report["candidate"], "microgpt_block_slice")
